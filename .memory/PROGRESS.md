...
- 2025-11-09: Завершены сабтаски 4.5.9c1–4.5.9c3 — добавлены интеграционные тесты `tests/api/admin/test_settings.py`, проверены TTL обновления, ошибки прав и аудит ротации пароля (pytest skip при отсутствии FastAPI задокументирован).
...
- 2025-11-09: Сабтаски 4.5.9a–4.5.9b закрыты — `GET/PUT /api/settings` используют SettingsService, доменные модели маппятся в Pydantic схемы, исключения 400/403/422 обрабатываются согласно контракту.
- 2025-11-09: Проведена ревизия 4.5.9 — подтверждена корректность эндпоинтов `GET/PUT /api/settings`, TTL пересчитывается через SettingsService, ротация DSLR-пароля фиксируется в аудите, `pytest tests/api/admin/test_settings.py` отмечен skip из-за отсутствия fastapi в окружении.
