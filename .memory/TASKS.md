...
- [ ] **4. Фаза 4 — Реализация серверного приложения (IMPL)**
  ...
  - *Задачи:*
    ...
    - [ ] 4.3 Разработать воркеры обработки задач.
      ...
      - [ ] 4.3.1 Реализовать запуск пула из четырёх асинхронных воркеров как фоновых задач FastAPI (lifespan/background task), учитывая `QUEUE_MAX_IN_FLIGHT_JOBS` и регистрацию задач в одном event loop.
      ...
      - [ ] 4.3.9 Обновить DI и `create_app`, чтобы при старте приложения автоматически поднимались четыре фоновые задачи воркеров c Postgres-очередью и чтобы shutdown корректно останавливал пул.
      ...
      - [ ] 4.3.12 Переписать `QueueWorker` на чистую `asyncio`-модель без вложенных `asyncio.run`: добавить конфиг ретраев (5 попыток, пауза 3 с, таймаут 5 с), поддержку отмены активных задач и закрытие `httpx.AsyncClient` при shutdown.
