...
- [x] **4. Фаза 4 — Реализация серверного приложения (IMPL)**
  ...
  - *Задачи:*
    ...
    - [ ] 4.4 Настроить хранение результатов и публичные ссылки.
      ...
      - [ ] 4.4.2 Обновить `JobService.finalize_job` для записи `result_*`, очистки `result_inline_base64` и фиксации TTL = 72h.
      - [ ] 4.4.3 Имплементировать `GET /public/results/{job_id}`: возвращать временную подписанную ссылку (302/307) на объектное хранилище до истечения TTL и `410 Gone` после.
      - [ ] 4.4.4 Настроить фонового очистителя результатов (запуск каждые 15 минут) и удаление файлов из `MEDIA_ROOT/results` ровно по достижении `result_expires_at`.
      - [ ] 4.4.5 Добавить тесты публичных ссылок и очистки (200→410, отсутствие утечек файлов/метаданных).
      - [ ] 4.4.6 Обновить документацию по структуре `MEDIA_ROOT`, TTL и процедурам очистки (фиксированный TTL 72h, расписание очистителя).
