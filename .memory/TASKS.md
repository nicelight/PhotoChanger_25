...
- [ ] **4. Фаза 4 — Реализация серверного приложения (IMPL)**
  ...
  - *Задачи:*
    ...
    - [ ] 4.3 Разработать воркеры обработки задач.
      ...
      - [ ] 4.3.9 Переписать `QueueWorker` на совместимую с асинхронными адаптерами модель (выделенный event loop либо `anyio`), чтобы избежать вложенных `asyncio.run` и обеспечить поддержку реальных HTTP-клиентов.
    - [ ] 4.4 Настроить хранение результатов и публичные ссылки.
      ...
      - [ ] 4.4.7 Реализовать server-side helper для конвертации `result_inline_base64` в файл под `MEDIA_ROOT/results` и повторно использовать его в `QueueWorker._materialize_provider_result`.
    - [ ] 4.5 Разработать Admin API.
      ...
      - [ ] 4.5.9 Реализовать `StatsService` и инфраструктурный слой агрегации (`stats_repository`) для поддержки `/api/stats/*`.
