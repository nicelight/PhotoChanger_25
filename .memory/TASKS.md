...
- [x] **4. Фаза 4 — Реализация серверного приложения (IMPL)**
  ...
  - *Задачи:*
    ...
    - [ ] 4.5 Развернуть административную платформу (домен, безопасность, API).
      ...
      - [x] 4.5.4 Реализовать `SlotService`: CRUD слотов, валидация операций и `settings_json`, привязка шаблонных медиа, поддержка ETag/If-Match. _(Review 2025-11-02: текущий `SlotManagementService` обращается к `SettingsService.read_settings()`, метода нет — требуется скорректировать реализацию/контракт на `get_settings`.)_
