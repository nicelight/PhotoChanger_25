...
- [ ] **4. Фаза 4 — Реализация серверного приложения (IMPL)**
  ...
  - *Задачи:*
    ...
    - [ ] 4.2 Реализовать очередь на PostgreSQL.
      - [ ] ДЛЯ РАЗМЫШЛЕНИЙ: текущая реализация `PostgresJobQueue` создаёт схему «на лету», поэтому без миграционного инструмента будет сложно контролировать schema drift между средами.
      - [ ] Вопрос консультанту: утвердить стек миграций для очереди (Alembic против SQL-скриптов) и формат версионирования таблиц `jobs`/`processing_logs`.
      ...
      - [ ] 4.2.5a Реализовать сбор количества активных задач и лимитов на уровне очереди/БД.
      - [ ] 4.2.5b Пробросить ограничения параллелизма в Ingest API/JobService для возврата 429/503.
      ...
      - [ ] 4.2.9 Подключить реальный драйвер PostgreSQL/пул соединений и транзакционный слой в очереди.
      - [ ] 4.2.10 ДЛЯ РАЗМЫШЛЕНИЙ: миграция `processing_logs` критична для 4.3.6 и 4.5.6; без неё статистика не заработает.
      - [ ] 4.2.11 Задать пользователю вопросы о требованиях к лимитам очереди и форматам back-pressure перед реализацией.
    - [ ] 4.3 Разработать воркеры обработки задач.
      - [ ] ДЛЯ РАЗМЫШЛЕНИЙ: `QueueWorker` вызывает провайдерские методы через `asyncio.run`, что может конфликтовать с lifespan FastAPI и не позволит использовать `httpx.AsyncClient` в Phase 5.
      - [ ] Вопрос консультанту: подтвердить модель запуска воркеров (количество экземпляров, отдельный event loop, требования к graceful shutdown).
      ...
      - [ ] 4.3.9 Обновить DI и `create_app`, чтобы очередь/воркер использовали реальные реализации вместо in-memory стабов.
      - [ ] 4.3.10 ДЛЯ РАЗМЫШЛЕНИЙ: `QueueWorker` сейчас использует `asyncio.run`; продумать отдельный поток/процесс для провайдера.
      - [ ] 4.3.11 Задать пользователю вопросы о целевом режиме исполнения воркеров (пул потоков/процессов, количество воркеров).
      - [ ] 4.3.12 Переписать `QueueWorker` на совместимую с асинхронными адаптерами модель (выделенный event loop либо `anyio`), чтобы избежать вложенных `asyncio.run` и обеспечить поддержку реальных HTTP-клиентов.

    - [ ] 4.4 Настроить хранение результатов и публичные ссылки.
      - [ ] ДЛЯ РАЗМЫШЛЕНИЙ: `DefaultJobService.refresh_recent_results` пока `NotImplemented`, а `DefaultMediaService` не конвертирует base64-ответы в файлы — без этого UI не получит галерею и ссылки скачивания.
      - [ ] Вопрос консультанту: подтвердить стратегию генерации превью (хранить base64 в БД или писать отдельные миниатюры на диск?).
      ...
      - [ ] 4.4.7 ДЛЯ РАЗМЫШЛЕНИЙ: удостовериться, что `MEDIA_ROOT` подготовлен и очиститель не удаляет payload до завершения ответа.
      - [ ] 4.4.8 Задать пользователю вопросы о требованиях к TTL публичных ссылок и форматам выдачи результатов.
      - [ ] 4.4.9 Реализовать server-side helper для конвертации `result_inline_base64` в файл под `MEDIA_ROOT/results` и повторно использовать его в `QueueWorker._materialize_provider_result`.

    - [ ] 4.5 Разработать Admin API.
      - [ ] ДЛЯ РАЗМЫШЛЕНИЙ: маршруты `/api/*` возвращают `endpoint_not_implemented`, а инфраструктурные репозитории (`stats_repository`, `settings_repository`, `slot_repository`) пока не подключены к сервисам.
      - [ ] Вопрос консультанту: уточнить требования к полям JWT (`permissions`/`exp`) и форматам ответов для UI (нужны ли пагинация и фильтры?).
      ...
      - [ ] 4.5.9 Реализовать `StatsService`, репозиторий агрегатов и интеграцию с `processing_logs`.
      - [ ] 4.5.10 ДЛЯ РАЗМЫШЛЕНИЙ: синхронизировать с задачами 4.6.* по хранению хэшей и JWT, чтобы `/api/login` был консистентен.
      - [ ] 4.5.11 Задать пользователю вопросы об ожиданиях по статистическим метрикам и административному UX.
    - [ ] 4.6 Добавить авторизацию и безопасность.
      ...
      - [ ] 4.6.6 ДЛЯ РАЗМЫШЛЕНИЙ: выбрать и задокументировать алгоритм хэширования (`bcrypt` vs `pbkdf2`) и параметры усиления.
    ...
    - [ ] 4.7 Внедрить логирование и метрики.
      ...
      - [ ] 4.7.6 ДЛЯ РАЗМЫШЛЕНИЙ: определить стек метрик (Prometheus/Pushgateway/StatsService) до начала интеграции.
    ...
    - [ ] 4.8 Синхронизировать артефакты и решения Фазы 4.
      ...
      - [ ] 4.8.4 ДЛЯ РАЗМЫШЛЕНИЙ: запланировать обновление OpenAPI после внедрения Admin API и публичных ссылок.
    ...
    - [ ] 4.9 Восстановить тестовое окружение и CI.
      ...
      - [ ] 4.9.3 ДЛЯ РАЗМЫШЛЕНИЙ: подготовить docker-compose/инструкции для локального Postgres под новые интеграционные тесты.
...
- [ ] **5. Фаза 5 — Интеграция провайдеров и UI**
  ...
  - *Задачи:*
    - [ ] 5.1 Интегрировать адаптер Gemini.
      ...
      - [ ] 5.1.1 Реализовать `GeminiProviderAdapter.prepare_payload`, маппя `slot.settings_json` и контекст `Job` в структуру `contents`/`inline_data`/`file_data` по `Docs/providers/gemini.md`.
      - [ ] 5.1.2 Настроить `httpx.AsyncClient` (или официальную SDK) в `submit_job`, брать API-ключ из `AppConfig.provider_keys['gemini']`, прокидывать `x-goog-api-key`, логировать correlation-id и latency.
      - [ ] 5.1.3 Реализовать helper для Files API: загрузка больших payload в `files.upload`, кеширование `file_uri` и ревокация по истечении TTL (интегрировать с `MediaService`).
      - [ ] 5.1.4 Имплементировать `poll_status`/`cancel` с обработкой статусов `succeeded`/`processing`/ошибок, ретраями на 429/5xx и маппингом ошибок в `JobFailureReason`.
      - [ ] 5.1.5 Расширить `QueueWorker._materialize_provider_result` и новый helper из 4.4.7, чтобы результат Gemini сохранялся в `MEDIA_ROOT/results`, вычислялся checksum, выставлялись `result_file_path`/`result_mime_type` и очищался `result_inline_base64` после выдачи ingest.
      - [ ] 5.1.6 Добавить unit/integration тесты адаптера (моки Gemini), покрыть success/pending/timeout/error, обновить `tests/integration/test_queue_worker_dispatch.py` и фикстуры.
      - [ ] 5.1.7 Обновить документацию (`Docs/providers/gemini.md`, runbook ingest) по ключам, квотам, процедурам отладки и лимитам Files API.
    - [ ] 5.2 Интегрировать адаптер Turbotext.
      ...
      - [ ] 5.2.1 Реализовать `TurbotextProviderAdapter.prepare_payload`, формируя HTTPS-ссылки на `media_object` и параметры операций из `slot.settings_json`.
      - [ ] 5.2.2 Имплементировать `submit_job`/`poll_status`/`cancel` по REST `api_ai`, сохранять `queueid`, обрабатывать статусы `reconnect`/`success`/`error`, учитывать `T_sync_response`.
      - [ ] 5.2.3 Добавить защиту от превышения дедлайна: синхронизация с `QueueWorker` для досрочной отмены задач, очистка payload и фиксация `failure_reason`.
      - [ ] 5.2.4 Покрыть интеграционными тестами адаптер (успех, billing error, timeout), обновить `tests/mocks/turbotext` и e2e сценарии.
      - [ ] 5.2.5 Обновить документацию (`Docs/providers/turbotext.md`) и operational runbook по настройке ключей, биллинга и SLA.
    - [ ] 5.3 Завершить административный UI.
      ...
      - [ ] 5.3.1 Реализовать клиентский фасад (`src/app/ui/facade.py`) для вызовов `/api/login`, `/api/settings`, `/api/slots/*`, `/api/stats/*`, хранить JWT в cookies/сессии.
      - [ ] 5.3.2 Переписать шаблоны `slots/index.html` и `slots/detail.html` на HTMX/JS-запросы к Admin API, удалить `mock_data` и обеспечить загрузку провайдерских настроек.
      - [ ] 5.3.3 Добавить формы редактирования слота (provider/operation/settings_json) с отображением ошибок из 422 ответов и подтверждением изменений.
      - [ ] 5.3.4 Реализовать галерею `recent_results`, используя `GET /api/slots/{slot_id}`, подсвечивать элементы с истёкшим TTL и предоставлять кнопки скачивания.
      - [ ] 5.3.5 Наполнить `/ui/stats` реальными данными из `/api/stats/*`, добавить агрегированные карточки и графики (Chart.js или SVG), обработать пустые состояния.
      - [ ] 5.3.6 Добавить smoke/e2e-тесты UI (pytest -m e2e или Playwright) для сценариев логина, редактирования слота и просмотра результатов.
      - [ ] 5.3.7 Обновить документацию (`Docs/frontend-examples`, README) с описанием потоков авторизации, обновления слотов и просмотра статистики.
    - [ ] 5.4 Реализовать публичные страницы скачивания.
      ...
      - [ ] 5.4.1 Реализовать `GET /public/media/{media_id}` и `GET /public/results/{job_id}` с потоковой выдачей файлов из `MEDIA_ROOT`, проверкой `result_expires_at` и логированием обращений.
      - [ ] 5.4.2 Добавить шаблоны/эндпоинты UI для публичного скачивания (HTMX/Vanilla JS), показывать состояние «срок истёк» и инструкцию по повторному ingest.
      - [ ] 5.4.3 Написать unit и contract тесты (200/404/410, повторные скачивания, истечение TTL) с использованием временных файлов и мока очереди.
      - [ ] 5.4.4 Обновить public runbook/README с примерами URL, политикой TTL, рекомендациями по CDN/кешированию и шагами очистки.
