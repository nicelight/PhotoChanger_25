...
## TODO
- [ ] **0. Фаза 0 — Аналитика и синхронизация SDD**
  - *Цель:* убедиться, что артефакты SDD и бриф согласованы перед началом реализации.
  - *Задачи:*
    - [ ] 0.1 Сверить артефакты брифа и SDD.
      - [ ] 0.1.1 Сопоставить `Docs/brief.md` с `spec/docs/blueprints/*` по сущностям, сценариям и ограничениям.
      - [ ] 0.1.2 Сравнить `Docs/brief.md` с `spec/contracts/openapi.yaml` по эндпоинтам и статусам, отметить расхождения для цикла Re-Sync.
      - [ ] 0.1.3 Обновить чек-лист контроля в `agents.md` с учётом выявленных расхождений.
    - [ ] 0.2 Подтвердить актуальность ключевых SDD-артефактов.
      - [ ] 0.2.1 Перепроверить vision/context/domain-model/test-plan согласно `Docs/sdd_roadmap.md`.
      - [ ] 0.2.2 Зафиксировать требуемые ревью, список ответственных и сроки актуализации.
...
- [ ] **1. Фаза 1 — Контракты и схемы (SPEC REVIEW)**
  ...
  - *Задачи:*
    ...
    - [ ] 1.4 Подготовить архитектурные решения по TTL и очистке медиа.
      - [ ] 1.4.1 Проверить наличие актуальных ADR по TTL, polling БД и очистке медиа.
      - [ ] 1.4.2 При необходимости подготовить обновления ADR или зафиксировать подтверждение актуальности.
  - *Выходные артефакты:* протокол ревью контрактов, ADR по ключевым решениям или подтверждение актуальности.

- [ ] **2. Фаза 2 — Генерация стабов и scaffolding (STUBS GEN)**
  - *Цель:* подготовить минимально жизнеспособные каркасы API/доменных сервисов.
  - *Задачи:*
    - [ ] 2.1 Обновить каркасы, сгенерированные из OpenAPI.
      - [ ] 2.1.1 Сгенерировать или обновить контроллеры, DTO и клиентские типы.
      - [ ] 2.1.2 Сопоставить с существующими стабами и удалить устаревшие элементы.
    - [ ] 2.2 Провести аудит доменных сервисов и репозиториев.
      - [ ] 2.2.1 Проверить каркасы `JobService`, `SlotService`, `SettingsService`, `MediaService`, `StatsService`, `QueueWorker`.
      - [ ] 2.2.2 Составить backlog недостающих интерфейсов и зависимостей.
    - [ ] 2.3 Подготовить инфраструктурные заглушки.
      - [ ] 2.3.1 Настроить очередь на PostgreSQL с `SELECT … FOR UPDATE SKIP LOCKED`.
      - [ ] 2.3.2 Настроить стораджи медиа (`MEDIA_ROOT`) и провайдерские адаптеры.
      - [ ] 2.3.3 Актуализировать конфигурацию UI.
    - [ ] 2.4 Обновить UI scaffolding без бизнес-логики.
      - [ ] 2.4.1 Пересобрать страницы списка слотов и настройки слота.
      - [ ] 2.4.2 Подготовить страницы статистики и просмотра результатов с заглушечными данными.
...
- [ ] **3. Фаза 3 — Тестирование контрактов и инфраструктуры (CONTRACT TESTS GREEN)**
  ...
  - *Задачи:*
    - [ ] 3.1 Построить контрактные тесты API.
      - [ ] 3.1.1 Покрыть ingest, админ-API и публичные ссылки позитивными сценариями.
      - [ ] 3.1.2 Добавить негативные сценарии и проверки ошибок валидации.
    - [ ] 3.2 Настроить интеграционные тесты очереди.
      - [ ] 3.2.1 Смоделировать постановку `Job`, работу воркера и соблюдение `expires_at`.
      - [ ] 3.2.2 Проверить ручную отмену и очистку просроченных задач.
    - [ ] 3.3 Разработать моки провайдеров.
      - [ ] 3.3.1 Реализовать режимы «успех», «таймаут», «ошибка» для Gemini.
      - [ ] 3.3.2 Реализовать аналогичные режимы и ограничения для Turbotext.
