  ...
  - *Задачи:*
    ...
    - [ ] 4.5 Развернуть административную платформу (домен, безопасность, API).
      ...
      - [x] 4.5.3 Реализовать `SettingsService`: чтение/обновление конфигурации, ротация ingest-пароля, пересчёт TTL и аудит изменений.
        ...
        - [ ] 4.5.3d Устранить конфликт dataclass `SlotTemplateDTO` (non-default поле после default) в `src/app/schemas/__init__.py`, чтобы разблокировать `pytest tests/unit/services/test_settings_service.py`.
      - [ ] 4.5.4 Реализовать `SlotService`: CRUD слотов, валидация операций и `settings_json`, привязка шаблонных медиа, поддержка ETag/If-Match. _(Review 2025-11-02: текущий `SlotManagementService` обращается к `SettingsService.read_settings()`, метода нет — требуется скорректировать реализацию/контракт на `get_settings`.)_
      ...
      - [ ] 4.5.5 Реализовать `StatsService`: агрегация по `processing_logs`, расчёт дашбордов, `recent_results`, кеширование и API для метрик. _(Review 2025-11-02: `CachedStatsService` использует единый TTL и не реализует требование кеша 5 мин для слотов и 1 мин для глобальной статистики, нужно разделить политики.)_
        - [ ] 4.5.5a Спроектировать агрегаты и модель статистики (KPI, окна, TTL кеша). _(Утверждено 2025-10-31: slot stats ≤31 дней, default range = последние 14 дней с `group_by=day`; global ≤90 дней, default `group_by=week`; кеш слотов 5 мин, глобального среза 1 мин.)_
