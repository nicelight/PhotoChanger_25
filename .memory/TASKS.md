...
- [x] **4. Фаза 4 — Реализация серверного приложения (IMPL)**
  ...
  - *Задачи:*
    ...
    - [ ] 4.5 Развернуть административную платформу (домен, безопасность, API).
      - [ ] 4.5.R1 REFLECT — зафиксировать недостающие компоненты (репозитории, сервисы, безопасность) и риски запуска Admin API.
        - [ ] 4.5.R1a Проанализировать реализации в `src/app/services/`, `src/app/repositories/`, `src/app/security/` на предмет заглушек и отсутствующих компонентов.
        - [ ] 4.5.R1b Сопоставить требования Admin API из `spec/contracts/openapi.yaml` и `.memory/USECASES.md` с текущим кодом, выделить пробелы.
        - [ ] 4.5.R1c Зафиксировать риски (схема БД, безопасность, интеграция со статистикой) в `.memory/WORKLOG.md` и предложить план синхронизации с CONSULT-задачами.
      - [ ] 4.5.C1 CONSULT — согласовать с тимлидом набор JWT-claim'ов, формат ответов (пагинация/фильтры) и минимальные статистики для UI.
        - [ ] 4.5.C1a Свести требования к авторизации и CLAIM'ам из `spec/contracts/openapi.yaml`, `Docs/brief.md` и текущих DTO.
        - [ ] 4.5.C1b Подготовить варианты структур ответов (пагинация, фильтры, набор статистик) с аргументами.
        - [ ] 4.5.C1c Сформулировать вопросы для тимлида, согласовать решения и обновить записи в `.memory/WORKLOG.md` и `.memory/TASKS.md`.
      - [ ] 4.5.1 Подготовить миграции и схему хранения для `admin_settings`, `slots`, `slot_templates`, агрегатов статистики, обеспечить индексацию по ключевым полям.
        - [ ] 4.5.1a Спроектировать модель данных Admin API (сущности, связи, атрибуты).
        - [ ] 4.5.1b Определить индексацию и ограничения для таблиц (`status`, `expires_at`, `slot_id` и др.).
        - [ ] 4.5.1c Реализовать Alembic-миграции с `upgrade`/`downgrade` и smoke-тестом.
        - [ ] 4.5.1d Обновить фикстуры/seed-данные и документацию схемы.
      - [ ] 4.5.2 Реализовать инфраструктурные репозитории (`SettingsRepository`, `SlotRepository`, `TemplateMediaRepository`, `StatsRepository`) на SQLAlchemy и покрыть unit-тестами.
        - [ ] 4.5.2a Уточнить интерфейсы и возвращаемые модели репозиториев.
        - [ ] 4.5.2b Реализовать репозитории с учётом транзакций, фильтров и обработки ошибок.
        - [ ] 4.5.2c Написать unit-тесты на CRUD, пагинацию и ошибки репозиториев.
      - [ ] 4.5.3 Реализовать `SettingsService`: чтение/обновление конфигурации, ротация ingest-пароля, пересчёт TTL и аудит изменений.
        - [ ] 4.5.3a Спроектировать публичные методы сервиса и их контракт.
        - [ ] 4.5.3b Имплементировать бизнес-логику (транзакции, шифрование, аудит).
        - [ ] 4.5.3c Покрыть сервис тестами и обновить документацию по политике настройки.
      - [ ] 4.5.4 Реализовать `SlotService`: CRUD слотов, валидация операций и `settings_json`, привязка шаблонных медиа, поддержка ETag/If-Match.
        - [ ] 4.5.4a Определить правила валидации, ограничения провайдеров и архивирования.
        - [ ] 4.5.4b Реализовать CRUD-операции со связями и проверками целостности.
        - [ ] 4.5.4c Внедрить ETag/If-Match и обработку конфликтов обновлений.
        - [ ] 4.5.4d Покрыть сервис тестами (CRUD, валидации, ETag, архивирование).
      - [ ] 4.5.5 Реализовать `StatsService`: агрегация по `processing_logs`, расчёт дашбордов, `recent_results`, кеширование и API для метрик.
        - [ ] 4.5.5a Спроектировать агрегаты и модель статистики (KPI, окна, TTL кеша).
        - [ ] 4.5.5b Реализовать методы агрегации и кеширования с инвалидацией.
        - [ ] 4.5.5c Интегрировать события от очереди/воркеров для обновления статистики.
        - [ ] 4.5.5d Протестировать расчёт метрик, кеш и поведение на крайних сценариях.
      - [ ] 4.5.6 Подключить новые сервисы и репозитории в DI-контейнер, интегрировать `QueueWorker`/`JobService` с публикацией событий для `StatsService`.
        ...
        - [ ] 4.5.6c Добавить smoke-тест и актуализировать конфигурацию окружений.
      - [ ] 4.5.C2 CONSULT — уточнить у тимлида ожидаемый горизонт статистики (по слотам, скользящие окна) и структуру `recent_results`.
        - [ ] 4.5.C2a Подготовить варианты агрегатов и структуры `recent_results`.
        - [ ] 4.5.C2b Сформулировать вопросы по фильтрам, лимитам и SLA обновления.
        - [ ] 4.5.C2c Зафиксировать решения, скорректировать задачи 4.5.5/4.5.12 и обновить артефакты.
      ...
      - [ ] 4.5.12 Собрать `/api/stats/global`, `/api/stats/{slot_id}`, `/api/stats/{slot_id}/recent_results` с параметрами диапазона дат и лимитов.
        - [ ] 4.5.12a Уточнить контракт эндпоинтов (фильтры, лимиты, формат `recent_results`).
        ...
        - [x] 4.5.12d Свести `SlotStatsMetric` и `GlobalStatsMetric` к общей схеме `StatsMetricBase`, чтобы контракты статистики использовали единый набор счётчиков и формат периодов.
      ...
      - [ ] 4.5.14 Обновить документацию (flow JWT, матрица прав, runbook администрирования, примеры запросов) и OpenAPI при необходимости.
        ...
        - [ ] 4.5.14c Проверить согласованность документации с Docs/README и внести правки.
      - [ ] 4.5.Q1 Задать пользователю вопросы об ожиданиях по административному UX, частоте обновления статистики и форматах экспорта.
        - [ ] 4.5.Q1a Составить перечень тем (обновление статистики, экспорт данных, UX-ожидания).
        - [ ] 4.5.Q1b Сформулировать вопросы и провести обсуждение с пользователем.
        - [ ] 4.5.Q1c Зафиксировать ответы и обновить связанные задачи/артефакты.
...
- [ ] **5. Фаза 5 — Интеграция провайдеров и UI**
  ...
  - *Задачи:*
    - [ ] 5.1 Интегрировать адаптер Gemini. — *(Сложность: высокая; ветвления: требует согласования асинхронной архитектуры воркеров и политики хранения файлов.)*
      ...
      - [ ] 5.1.1 Реализовать `GeminiProviderAdapter.prepare_payload`, маппя `slot.settings_json` и контекст `Job` в структуру `contents`/`inline_data`/`file_data` по `Docs/providers/gemini.md`. — *(Сложность: средняя; ветвления: необходимо согласовать единый формат контента между провайдерами, иначе придётся разветвлять модель `ProviderPayload`.)*
      ...
      - [ ] 5.1.7 Обновить документацию (`Docs/providers/gemini.md`, runbook ingest) по ключам, квотам, процедурам отладки и лимитам Files API. — *(Сложность: низкая; ветвления: важно синхронизировать с будущими ADR, чтобы избежать расхождения по SLA.)*
    ...
    - [ ] 5.2 Интегрировать адаптер Turbotext. — *(Сложность: высокая; ветвления: влияет на модель биллинга, хранение публичных ссылок и дедлайны очереди.)*
      ...
      - [ ] 5.2.5 Обновить документацию (`Docs/providers/turbotext.md`) и operational runbook по настройке ключей, биллинга и SLA. — *(Сложность: низкая; ветвления: важно синхронизировать с финансовыми ограничениями и политиками из 6.*.)*
    ...
    - [ ] 5.3 Завершить административный UI. — *(Сложность: высокая; ветвления: определяет стек фронтенда, модель авторизации и потоков данных с Admin API.)*
      ...
      - [ ] 5.3.7 Обновить документацию (`Docs/frontend-examples`, README) с описанием потоков авторизации, обновления слотов и просмотра статистики. — *(Сложность: средняя; ветвления: документация должна зафиксировать выбранную архитектуру UI, иначе появятся расхождения с реализацией.)*
