...
- **Основной поток:**
  ...
  2. Возврат JWT со scope `admin` (TTL 168 ч), токен сохраняется в localStorage UI.
...
- **Критерии успеха:** JWT валиден 168 ч, audit запись фиксируется, пароль вступает в силу сразу.
- **Ошибки:** `401` (неверный логин/пароль или отсутствует токен), `403` (scope ≠ `admin`), `429` (частые попытки).

## UC1. Настройка слота администраторами
- **Акторы:** Администратор, Slot Management UI/API.
- **Предусловия:** Авторизован JWT со scope `admin`.
