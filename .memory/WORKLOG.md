...
## BUGFIX — ingest пароль после апгрейда (2025-12-03)
- 2025-12-03 12:25 — Проверил инструктаж (MISSION/CONTEXT/TASKS и др.), изучил жалобу Codex о том, что после перехода с `ingest_password_hash` на plaintext `ingest_password` проверки пароля обходятся (expected=""). Добавил fallback на sha256-хэш в `IngestService.verify_ingest_password`, прокинул legacy hash из SettingsService и покрыл unit-тестами (ингест сервис и загрузка настроек из legacy стора).
...
## FEAT PHC-3.2 — UX слотов (2025-11-11)
- 2025-11-11 02:02 — Перед стартом T PHC-3.2.5 перечитал `.memory/MISSION.md`, `.memory/CONTEXT.md`, `.memory/TASKS.md`, `.memory/ASKS.md`, `.memory/DECISIONS.md`, `.memory/USECASES.md`, `.memory/INDEX.yaml`, а также `docs/BRIEF.md`, `docs/PRD.md`, `docs/ARCHITECTURE.md`, чтобы освежить требования к UX слотов и API.
- 2025-11-11 02:06 — Обновил `.memory/TASKS.md`: добавлен блок `US PHC-3.2.5.GOV` с REFLECT/CONSULT подзадачами перед исполнением `T PHC-3.2.5`, чтобы зафиксировать вопросы по интеграции форм с backend.
- 2025-11-11 02:12 — REFLECT (T PHC-3.2.5.GOV.1): текущие страницы `frontend/slots/*.html` не подключают `AdminAuth`, поэтому любые запросы `/api/slots/*`/`/api/template-media/register`/`/api/slots/{slot_id}/test-run` теперь получают 401 сразу после включения JWT. Нужно решить: (1) добавляем общий `auth.js` и заставляем слот-страницы требовать токен перед инициализацией; (2) меняем `slot-api.js` на использование `AdminAuth.authFetch` (иначе нет заголовка Authorization и автологина при 401). Дополнительно выяснилось, что `loadRecentResults` использует `GET /api/slots/{slot_id}` для галереи, но не заполняет форму реальными данными — страницу нужно гидратировать серверной конфигурацией, чтобы название/операция/промпт не расходились с БД. Для `POST /api/slots/{slot_id}/test-run` backend ожидает `slot_payload` (JSON overrides), а мы отправляем разрозненные поля без авторизации, то есть тестовый запуск не подхватывает несохранённые изменения. Узкие места: приходится править и шаблон `template.html`, и 15 копий `slot-XXX.html` (нужно согласовать, можно ли автоматизировать), продумать UX обработки ошибок 422/409, а также убедиться, что загрузка шаблонных медиа работает с тем же токеном.
- 2025-11-11 02:15 — CONSULT (T PHC-3.2.5.GOV.2): вопросы к тимлиду: (1) подтверждаем, что все страницы слотов должны требовать JWT (т.е. подключаем `auth.js`, редиректим при 401 и используем `AdminAuth.authFetch` для PUT/GET/POST); (2) нужно ли на клиенте обязательно подгружать актуальную конфигурацию через `GET /api/slots/{slot_id}` и синхронизировать заголовок/название/промпт перед редактированием, или достаточно данных из статичного HTML; (3) Тестовый запуск — формируем `slot_payload` из несохранённых изменений (provider/operation/settings/template_media) и отправляем его; верно ли, что это поведение соответствует ожиданиям PRD?; (4) допустимо ли для 15 HTML-страниц применить массовое обновление (скриптом) от шаблона, чтобы не править каждую вручную.
- 2025-11-11 02:20 — Тимлид подтвердил все предложенные шаги: slot-страницы требуют JWT и используют `AdminAuth`, UI обязан получать слот через `GET /api/slots/{slot_id}`, тестовый запуск отправляет `slot_payload` с текущими правками, массовое обновление HTML допустимо. Добавлены подзадачи `T PHC-3.2.5.1…3.2.5.4` в `.memory/TASKS.md`.
- 2025-11-11 02:32 — T PHC-3.2.5.1: подключил `AdminAuth` к SlotPage — `slot-config.js` теперь сохраняет ссылку на auth-модуль, `slot-main.js` требует JWT перед инициализацией и прерывает работу при отсутствии токена. Все Fetch-запросы `slot-api.js` переведены на `AdminAuth.authFetch`, а шаблон страниц подключает `/ui/static/admin/assets/auth.js`.
- 2025-11-11 02:50 — T PHC-3.2.5.2: добавил инициализацию слота из GET `/api/slots/{slot_id}` — `slot-api.js` теперь bootstrap’ит форму (display_name, провайдер, операция, prompt, template_media), синхронизирует селекты и выводит актуальный список результатов без дополнительного запроса. `slot-main.js` вызывает новый `bootstrapSlotFromServer` после привязки UI, а кнопка «Обновить» использует тот же эндпоинт.
- 2025-11-11 03:05 — T PHC-3.2.5.3: тестовый запуск теперь отправляет `slot_payload` с текущими правками (provider, operation, settings, template_media) и работает через авторизованный `authFetch`. Убраны устаревшие multipart поля (`prompt`, `provider`, `operation`), FormData содержит только `slot_payload` и `test_image`, что совпадает с серверным API.
- 2025-11-11 03:20 — T PHC-3.2.5.4: синхронизировал `frontend/slots/template.html` и все `slot-XXX.html` — добавил `data-login-path`/`data-dashboard-path`, подключил `/ui/static/admin/assets/auth.js`, чтобы страницы корректно работали с JWT и знали, куда редиректить при 401. Для 15 страниц применён скрипт `tmp_update_slots.py` (одноразовый), затем удалён.
- 2025-11-11 03:28 — REFLECT (T PHC-3.2.6.GOV.1): проверил текущую обработку 422 — в слот-UI `slot-api.js` уже вызывает `dom.applyFieldErrors`, но `FIELD_MAP` покрывает лишь несколько полей и нет визуальной подсказки (класс `.has-error` есть, а текст ошибки хранится в `dataset.errorMessage`, но нигде не выводится). Для settings-UI 422 вообще не разбираются: ответ `/api/settings` отображается как общий статус без подсветки конкретных input'ов. Бэкенд слотов отдаёт pydantic detail с `loc`, Settings API — тоже pydantic/HTTPException (например, `sync_response_seconds` вне диапазона). Нужно определиться: (1) показываем ли текст ошибки рядом с полем или достаточно рамки? (2) Как маппить nested поля (`settings.prompt`, `provider_keys.gemini`) на DOM. (3) Должен ли UI очищать подсветку при повторном вводе и при успешном ответе (сейчас `dom.clearFieldErrors` вызывается только в слотах, на settings его нет). Без этих решений UX остаётся неявным.
- 2025-11-11 03:34 — CONSULT (T PHC-3.2.6.GOV.2): приняли KISS-решение без эскалации — подсветка оформляется только рамкой (`.has-error`), тексты ошибок выводим минимально (через атрибут `title`), nested поля маппим простыми правилами (`provider_keys.*`, `slot_payload.*`), подсветку сбрасываем перед каждым запросом. Settings UI также должен разбирать 422 и подствечивать нужные input'ы.
- 2025-11-11 03:45 — Реализовал подсветку для slot UI: `dom.applyFieldErrors` теперь нормализует `loc` (убирает `body`), сохраняет сообщения, а `FIELD_MAP` в `slot-api.js` включает `slot_payload.*`; `clearFieldErrors` сбрасывает `dataset.errorMessage`.
- 2025-11-11 03:55 — Настройки UI получают аналогичный функционал: добавлены `clearFieldErrors`/`applyFieldErrors`, динамический маппинг `provider_keys.*`, обработка 422 с сообщением «Исправьте выделенные поля», а в `admin.css` появился стиль `.has-error` (красная рамка + подчёркивание). После успешного сохранения подсветка очищается.
- 2025-11-11 04:05 — Исправил UX настройки провайдеров: `renderProviderKeys` теперь всегда выводит инпуты минимум для базовых провайдеров (`gemini`, `turbotext` или список из `data-providers`), даже если бекенд вернул пустой объект. Это позволяет вводить ключи на чистой установке.
- 2025-11-11 03:30 — Добавил `US PHC-3.2.6.GOV` с REFLECT/CONSULT подзадачами в `.memory/TASKS.md`, согласно политике GOV перед любой реализацией UX.
- 2025-12-04 11:40 — Добавил `latest_result` в ответ `/api/slots/{slot_id}`, обновил Pydantic-схемы и OpenAPI, поднял версию контрактов до 0.9.2 (поддержка галереи).
- 2025-12-04 12:20 — Перенёс ленту результатов слотов в отдельную страницу `frontend/admin/gallery.html` (15 запросов к /api/slots/{id} со спойлерами), добавил ссылку/иконку галереи в навбар админки и оставил на слот-страницах только последний результат + CTA на галерею.
- 2025-12-05 11:30 — Реализован EP PHC-7: временная публичная галерея `/pubgallery` с in-memory TTL 15м, rate limit и кэшем; админская галерея получила кнопку «Расшарить на 15 минут», публичная страница `frontend/public/gallery.html`; OpenAPI/VERSION обновлены до 0.10.0.
...
- 2025-11-10 13:15 — Проанализировал макеты login/main из `spec/docs/ui/frontend-examples/`, текущие страницы `frontend/slots` и требования PRD к дашборду/настройкам.
- 2025-11-10 14:05 — Создал новый фронтенд `frontend/admin` (CSS + JS). Реализованы: login.html (форма входа в Auth API, хранение JWT в localStorage), dashboard.html (список слотов через /api/slots, копирование ingest ссылок, навигация) и settings.html (чтение/обновление глобальных настроек/ключей через /api/settings). Добавлен модуль auth.js и вспомогательные сценарии.

## ROADMAP — провайдеры (2026-01-09)
- 2026-01-09 04:10 — Обновил .memory/ROADMAP.md: зафиксирован KISS‑роадмап по провайдерам и минимальная стратегия логирования ошибок (error context).
- 2026-01-09 04:20 — ROADMAP: зафиксирован выбранный минимальный универсальный вариант логирования ошибок провайдеров.

## PHC-11 — GOV: KISS-расширяемость провайдеров (2026-01-09)
- 2026-01-09 04:35 — Зафиксировал ответы тимлида: driver checklist = минимум (ingest + prompt), порядок частей ingest→templates→prompt, output mime strict, общий минимум settings: model/prompt (output/template опционально), template_media = media_kind only, error context: provider/model/http_status/provider_error_message (<=300), логирование в ingest; ретраи оставить как есть — требуется REFLECT по текущей реализации.
- 2026-01-09 04:45 — CONSULT: оставляем role в template_media как внутреннюю подпись (без breaking-упрощения).
- 2026-01-09 05:05 — REFLECT: ретраи находятся на стороне драйверов (GeminiDriver: retry_policy + NO_IMAGE), ingest только wait_for; решение — зафиксировать требование аналогичных ретраев для всех драйверов в спеках.
- 2026-01-09 05:20 — PHC-11: пересобран блок FEAT PHC-11.2 под KISS-логирование без проброса контекста; задачи упрощены.
- 2026-01-09 05:30 — ROADMAP: обновил раздел логирования под KISS (без проброса контекста).
- 2026-01-09 05:45 — PHC-11.1.1/11.1.1a: добавил driver checklist и псевдокод сборки parts в spec/docs/providers/driver-checklist.md.
- 2026-01-09 05:55 — PHC-11.1.2/11.1.2b/11.1.2c: добавлены схемы slot.settings для Gemini 3.0 Flash и GPT‑5 (минимальный набор полей).
- 2026-01-09 05:55 — PHC-11.1.2a: подтверждено, что output/template опциональны (минимум model/prompt).
- 2026-01-09 05:55 — PHC-11.1.3: role в template_media оставлен обязательным (breaking не требуется).
- 2026-01-09 06:10 — PHC-11.1.4/11.1.4a/11.1.4b: зафиксированы ретраи на уровне драйверов в spec/docs/providers/gemini.md и spec/docs/providers/turbotext.md.
- 2026-01-09 06:25 — PHC-11.2: обновил provider docs (gemini/turbotext) с распределением логов, минимальными полями и запретом payload; отметил, что ingest логирует итоговый статус без дублей.
- 2026-01-09 06:40 — PHC-11.3: тесты драйверов уже покрывают success/timeout/provider_error/invalid_response и NO_IMAGE ретраи; задачи отмечены выполненными.
- 2026-01-09 06:55 — Обновил spec/contracts/VERSION.json до 0.11.1 и синхронизировал .memory/INDEX.yaml (схемы провайдеров, ретраи, логирование).
- 2026-01-09 07:05 — Зафиксирован старт разработки версии 2 в .memory/PROGRESS.md.
- 2026-01-09 07:20 — Добавил лог с URL админского дашборда при старте сервера (src/app/main.py).
- 2026-01-09 07:35 — Пересоздал .memory/ROADMAP.md: цели V2, контекст для драйверов gemini-3-pro-image-preview и gpt-image-1.5-2025-12-16, план интеграции.
- 2026-01-09 07:45 — ROADMAP: добавлена проработка UI‑опции выбора формата/размера результата при поддержке провайдера.

## GPT-image-1.5 — сбор официальных источников (2026-01-14)
- 2026-01-14 13:45 — Собрал данные из официальных docs OpenAI по GPT Image (base64), оформил Markdown в `spec/docs/providers/GPT-image-1.5/` и удалил временный парсер.

## Gemini 3 Pro Image Preview — сбор официальных источников (2026-01-14)
- 2026-01-14 14:20 — Собрал данные из официальных docs Google по `gemini-3-pro-image-preview` (base64 inline_data, responseModalities, imageConfig aspect_ratio/imageSize) и оформил Markdown в `spec/docs/providers/gemini-3-pro/`.

## EP PHC-12 — Драйверы Gemini 3 Pro / GPT Image 1.5 (2026-01-14)
- 2026-01-14 15:10 — Добавил схемы slot.settings для Gemini 3 Pro и GPT Image 1.5, обновил провайдерские справки (base64, imageConfig/size, ретраи/логирование).
- 2026-01-14 15:25 — Реализовал драйверы `Gemini3ProDriver` и `GptImage15Driver`, подключил их в фабрику провайдеров.
- 2026-01-14 15:40 — Обновил UI слотов: добавлены провайдеры, поля aspect_ratio/resolution и маппинг в settings.
- 2026-01-14 15:55 — Добавил unit‑тесты новых драйверов, прогон `py -X utf8 -m pytest tests/unit/providers`.
- 2026-01-14 16:05 — Обновил `spec/contracts/VERSION.json` до 0.12.0 и синхронизировал `.memory/INDEX.yaml`.

## EP PHC-13 — GOV: Gemini 1.5 aspect_ratio/resolution (2026-01-14)
- 2026-01-14 16:39 — Подготовил CONSULT: нужно подтвердить точный model_id Gemini 2.5 и поддержку imageConfig (aspectRatio/imageSize) до начала правок драйвера, схем и UI.

## EP PHC-13 — Gemini 2.5 imageConfig (2026-01-14)
- 2026-01-14 17:10 — Зафиксировал подтверждение требования от тимлида: включаем imageConfig (aspectRatio/imageSize) для Gemini 2.5, обновляем UI/схемы/тесты.
- 2026-01-14 17:20 — Обновил GeminiDriver: поддержка image_config → generationConfig.imageConfig, добавил provider/model/http_status/provider_error_message в логи и ограничение длины сообщения.
- 2026-01-14 17:30 — Обновил UI слотов: сохраняем aspect_ratio/resolution для Gemini 2.5 и показываем карточку imageConfig.
- 2026-01-14 17:40 — Обновил schema slot-settings-gemini + справку Gemini 2.5, дополнил unit‑тест GeminiDriver.
- 2026-01-14 17:45 — Прогнал `py -X utf8 -m pytest tests/unit/providers/test_gemini_driver.py` (8 passed).
- 2026-01-14 17:55 — Зафиксировал список aspect_ratio/resolution для Gemini 2.5 (совпадает с Gemini 3 Pro); отдельные лимиты imageConfig в документации не подтверждены.
- 2026-01-14 18:10 — Исправил битые строки в HTML слотов (aspect_ratio/resolution placeholder и подсказка), синхронизировал slot-*.html.
- 2026-01-14 18:35 — Уточнил сбор imageConfig: отправляем только при наличии aspect_ratio и resolution (UI и GeminiDriver), чтобы избежать INVALID_ARGUMENT.
- 2026-01-14 18:50 — Добавил логирование gemini.request.body (без inline_data) для диагностики INVALID_ARGUMENT.
- 2026-01-14 19:05 — Ограничил отправку imageConfig для GeminiDriver allowlist-ом моделей (2.5 не поддерживает), обновил тест и справку.
- 2026-01-14 19:20 — Убрал временное логирование gemini.request.body из GeminiDriver.
- 2026-01-14 19:30 — Синхронизировал список провайдеров в slot-*.html с template.html (добавлены gemini-3-pro и gpt-image-1.5).
- 2026-01-14 19:45 — Уточнил поддержку Gemini 2.5: отправляем только aspect_ratio (image_size не поддерживается), обновил UI, схему, справку и тесты, bump контрактов 0.13.1.
- 2026-01-14 19:55 — Прогнал `py -X utf8 -m pytest tests/unit/providers/test_gemini_driver.py` (8 passed).
- 2026-01-14 20:10 — UI: скрываю поле resolution для Gemini 2.5 (оставляем только aspect_ratio).
- 2026-01-14 20:25 — UI: скрываю селект операции для всех провайдеров, кроме Turbotext (автовыбор операции для остальных).
- 2026-01-14 20:40 — Settings UI: добавил GPT Image 1.5 в список провайдеров для ввода API ключа.
- 2026-01-14 20:55 — SettingsService: применяю provider_keys к runtime ключам (GEMINI/OPENAI/TURBOTEXT), добавил unit‑тест; bump контрактов 0.13.2.
- 2026-01-14 21:05 — Прогнал `py -X utf8 -m pytest tests/unit/settings/test_settings_service_ingest_upgrade.py` (2 passed).
- 2026-01-14 21:20 — SettingsService: очищаю runtime env при пустом ключе в provider_keys, обновил тест; bump контрактов 0.13.3.
- 2026-01-14 21:30 — Прогнал `py -X utf8 -m pytest tests/unit/settings/test_settings_service_ingest_upgrade.py` (2 passed).
- 2026-01-14 21:45 — Инициировал GOV для предложения /api/providers; добавил CONSULT/REFLECT задачи в .memory/TASKS.md.
- 2026-01-14 22:10 — REFLECT PHC-14: /api/providers снижает расхождения UI/бекенда и ускоряет ввод новых провайдеров, но добавляет контракт и тесты; ключевой риск — неверная версия манифеста в UI, решается статичным JSON в коде + кэширование на клиенте. Рекомендация: сделать минимальный read-only /api/providers, без кэша на сервере, UI читает на старте с fallback на slot-config.js.
