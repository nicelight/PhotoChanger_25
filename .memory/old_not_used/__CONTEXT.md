...
- **Local:** разработчики поднимают FastAPI-приложение и PostgreSQL (очередь) локально; медиа хранятся в файловой системе (`MEDIA_ROOT`), моки провайдеров подключаются через фикстуры. UI (HTML) работает на том же сервере.【F:/docs/PRD.md】【F:spec/docs/blueprints/test-plan.md】
...
- **Front:** Административный UI на HTML/Vanilla JS, использует REST-контракты (`/api/slots`, `/api/settings`, `/public/results/{job_id}`) для настройки слотов и просмотра `recent_results`. Примеры макетов — в `spec/docs/frontend-examples`.【F:/docs/PRD.md】【F:spec/docs/blueprints/use-cases.md】
