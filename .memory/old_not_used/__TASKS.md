...
## TODO
- [x] **2025-11-06 — Ops: убрать SQLite из стека**
  - [x] Удалить фоллбек на SQLite в `src/app/core/app.py`, чтобы отсутствие
    `psycopg` приводило к немедленному сбою.
  - [x] Перевести ORM-модели на `JSONB` без вспомогательных типов и сверить
    миграции Alembic.
  - [x] Переписать `tests/unit/repositories` на PostgreSQL-фикстуру и отказаться
    от `aiosqlite`.
  - [x] Обновить README, ops runbook и тестовую документацию о требовании
    PostgreSQL/`psycopg` и способах поднять локальную БД.
  - [x] Синхронизировать записи в `.memory/WORKLOG.md`, `.memory/PROGRESS.md` и
    актуализировать `TASKS`.
- [x] **4. Фаза 4 — Реализация серверного приложения (IMPL)**
  ...
  - *Задачи:*
    ...
    - [ ] 4.5 Развернуть административную платформу (домен, безопасность, API).
      ...
      - [x] 4.5.7 Реализовать `/api/login` с выдачей JWT (`permissions`, `exp`, `sub`), конфигурацией секретов и ограничениями TTL.
        - [x] 4.5.7a Спроектировать контракт `/api/login` (payload, ответ, claim'ы, TTL).
      - [x] 4.5.7b Реализовать эндпоинт FastAPI с проверкой credential и генерацией JWT.
        - [x] 4.5.7b1 Подготовить конфигурацию секретов/TTL: добавить `jwt_access_ttl_seconds` в `AppConfig`, обновить `configs/app.*.json`, определить источник хэшей администраторов (`secrets/runtime_credentials.json`).
        - [x] 4.5.7b2 Реализовать сервис аутентификации (`security/passwords.py`, `security/service.py`): загрузка хэшей, проверка логина/пароля, логирование и счётчик попыток.
        - [x] 4.5.7b3 Собрать `POST /api/login` в `src/app/api/routes/auth.py`: внедрить зависимость конфигурации/сервиса, формировать JWT (`permissions`, `sub`, `exp`) и ответы 200/401/429.
      - [x] 4.5.7c Покрыть авторизацию тестами (успех, ошибки, throttling).
          - [x] 4.5.7c1 Написать unit/contract тесты успешного логина: проверка структуры JWT, TTL и claim'ов.
          - [x] 4.5.7c2 Добавить тесты неуспешных сценариев (неизвестный пользователь, неверный пароль) и единообразных ошибок 401.
          - [x] 4.5.7c3 Реализовать тесты троттлинга `/api/login` (превышение лимитов, 429, сброс счётчиков).
      - [x] 4.5.8 Настроить `require_bearer_authentication`, проверку прав (`settings:write`, `slots:write`, `stats:read`), обработку 401/403 и троттлинг логина. _(Проверено 2025-11-09: unit тесты `AuthenticationService` проходят; интеграционные проверки FastAPI помечены `skip` из-за отсутствия зависимости в окружении.)_
        - [x] 4.5.8a Внедрить dependency/FastAPI security для Bearer-токена и обработку 401.
        - [x] 4.5.8b Реализовать матрицу прав и обработку 403 в эндпоинтах.
        - [x] 4.5.8c Настроить троттлинг `/api/login` и покрыть его тестами.
      - [x] 4.5.9 Имплементировать `GET/PUT /api/settings` с валидацией payload, ротацией ingest-пароля и триггером пересчёта TTL. _(Проверено 2025-11-09: эндпоинты возвращают snapshot/обновляют TTL и DSLR-пароль через SettingsService, ошибки 400/403/422 соответствуют контракту, интеграционные тесты `tests/api/admin/test_settings.py` покрывают сценарии обновления и ротации.)_
        - [x] 4.5.9a Подготовить Pydantic-схемы запросов/ответов и валидации.
        - [x] 4.5.9b Реализовать эндпоинты с интеграцией SettingsService и обработкой ошибок.
          - [x] 4.5.9b1 Добавить dependency FastAPI для получения `SettingsService` из `ServiceRegistry` и подготовить мапперы доменной модели в схемы ответа.
          - [x] 4.5.9b2 Реализовать обработчик `GET /api/settings`: авторизация, получение snapshot настроек, сериализация в контрактную схему, обработка ошибок хранилища.
          - [x] 4.5.9b3 Реализовать обновление `PUT /api/settings` для `sync_response_timeout_sec`: валидация payload, вызов `SettingsService.update_settings`, возврат пересчитанных TTL.
          - [x] 4.5.9b4 Реализовать ветку обновления/ротации DSLR-пароля через `SettingsService.rotate_ingest_password` с возвратом статуса и аудит-логированием ошибок.
          - [x] 4.5.9b5 Сопоставить исключения (`PermissionError`, `ValueError`, инфраструктурные ошибки) с ответами 400/401/403/422/500 и единообразными JSON-ошибками.
        - [x] 4.5.9c Написать тесты на обновление, ротацию пароля и пересчёт TTL.
          - [x] 4.5.9c1 Покрыть `GET /api/settings` unit/contract тестами на успешный сценарий и отсутствие прав.
          - [x] 4.5.9c2 Покрыть `PUT /api/settings` тестами на обновление `sync_response_timeout_sec`, пересчёт TTL и обработку валидационных ошибок.
          - [x] 4.5.9c3 Написать тесты ротации DSLR-пароля: успешный возврат секрета, запрет без прав и корректное логирование аудита.
      ...
      - [ ] 4.5.11 Имплементировать управление шаблонными медиа (upload/list/delete, привязка к слотам, проверка использования перед удалением).
        ...
        - [ ] 4.5.11b Реализовать эндпоинты с интеграцией TemplateMediaRepository/MediaService.
          - [ ] 4.5.11b1 Расширить `TemplateMediaRepository` полноценным CRUD (создание, листинг с фильтрами, удаление, привязки слотов).
          - [ ] 4.5.11b2 Проложить `TemplateMediaService`: валидация payload, управление файлами через `MediaService`, маппинг доменных ошибок.
          - [ ] 4.5.11b3 Подключить FastAPI-роуты upload/list/delete к сервису, реализовать схемы запросов/ответов и обработку ошибок.
        - [ ] 4.5.11c Добавить проверки использования медиа и покрыть API тестами.
          - [ ] 4.5.11c1 Реализовать проверку привязок к слотам/джобам перед удалением и сценарий force-delete согласно контракту.
          - [ ] 4.5.11c2 Написать тесты репозитория/сервиса/эндпоинтов (upload/list/delete, force, конфликты привязок, ограничения размера).
...
- [ ] **5. Фаза 5 — Интеграция провайдеров и UI**
  ...
  - *Задачи:*
    ...
    - [ ] 5.3 Завершить административный UI. — *(Сложность: высокая; ветвления: определяет стек фронтенда, модель авторизации и потоков данных с Admin API.)*
      ...
      - [ ] 5.3.2 Переписать шаблоны `slots/index.html` и `slots/detail.html` на HTML/JS-запросы к Admin API, удалить `mock_data` и обеспечить загрузку провайдерских настроек. — *(Сложность: высокая; ветвления: потребуется решить, использовать ли HTMX или полноценный SPA-фреймворк, что влияет на архитектуру фронтенда.)*
