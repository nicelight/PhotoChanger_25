...
с 504, фиксируя статус задачи. Временные артефакты (ingest-файлы и публичные ссылки) по‑прежнему используют это значение в качестве TTL, а итоговые изображения, сохранённые воркерами на диск, доступны 72 часа; подробности раскрыты в [доменной модели](../spec/docs/blueprints/domain-model.md).
...
создаёт `Job`, рассчитывает дедлайны (`expires_at`, `T_ingest_ttl`, `T_public_link_ttl`, `T_result_retention`) и ожидает ответ воркера до наступления `T_sync_response`, выполняя polling записи `Job` в БД примерно раз в секунду без дополнительных механизмов уведомлений.
...
Если провайдер Turbotext не возвращает результат до наступления `T_sync_response`, Ingest API отвечает 504 и немедленно завершает задачу: воркер не выполняет дальнейший polling, связанные временные ресурсы очищаются, публичные ссылки инвалидируются. Контракт ответа `components.responses.GatewayTimeout` в OpenAPI закрепляет, что Job финализирована и требует нового ingest-запроса для повторного запуска. При успешной обработке итоговый файл доступен по публичной ссылке `GET /public/results/{job_id}` до наступления `result_expires_at`.
...
- Возвращает `queueid`, который сохраняется в `Job.provider_job_reference` и используется воркером для polling в пределах `T_sync_response`.
...
- **Итоговые результаты** (`result_file_path`) сохраняются воркерами в `MEDIA_ROOT/results` на 72 часа (`T_result_retention`). TTL результата не связан с `job.expires_at`: как только задача финализирована, файл доступен весь период `result_expires_at = finalized_at + 72h`. Очиститель удаляет файл по достижении `result_expires_at`, обнуляет путь и делает публичную ссылку недоступной (`410 Gone`). UI и внешние клиенты используют `GET /public/results/{job_id}` для скачивания.
...
- **Воркеры** — исполняют операции провайдера, отслеживают `expires_at = created_at + T_sync_response`, не выходят за пределы общего дедлайна, сохраняют итоговые изображения с расчётом `result_expires_at = finalized_at + 72h` и гарантируют очистку временных данных.
...
- Для асинхронных сценариев Turbotext сохраняется `queueid`; воркер опрашивает `do=get_result` только в пределах `T_sync_response`. По истечении дедлайна задача завершается с `failure_reason = 'timeout'`, дальнейший polling не выполняется.
...
- [spec/contracts/openapi.yaml](../spec/contracts/openapi.yaml) — полный HTTP-контракт: эндпоинты ingest, слотов, настроек, медиа, статусов и публичной выдачи результатов (`GET /public/results/{job_id}`).
