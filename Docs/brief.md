...
Временные ссылки (`media_object`) живут фиксированное время — 60 секунд. Если за это окно провайдер не скачал файл, задача считается
...
- **Источники конфигурации:**
  - `T_sync_response` — `app_settings["ingest.sync_response_timeout_sec"]`, по умолчанию 48 секунд, верхняя граница 50 секунд.
  - `T_ingest_ttl` — `app_settings["media.ingest_ttl_sec"]`, по умолчанию 48 секунд; параметр всегда ≤ `T_sync_response` и определяет TTL исходных payload.
  - `T_media_limit` — жёсткий лимит конкретного временного хранилища (например, `MEDIA_PUBLIC_LINK_TTL_SEC = 60` секунд для публичных ссылок Turbotext).
- **Единый дедлайн Job:** при создании записи `Job` фиксируется поле `expires_at = created_at + T_job_deadline`, где `T_job_deadline = max(T_sync_response, T_media_limit_max)` и `T_media_limit_max` — максимальный лимит среди задействованных временных хранилищ (на данный момент 60 секунд для публичных ссылок). Значение хранится в БД и не меняется; API, воркеры и механизмы очистки получают дедлайн только через `DeadlineGuard`.
- **Политика TTL артефактов:** для всех временных объектов применяется формула `artifact_expires_at = min(created_at + T_media_limit, job.expires_at)`. Поскольку `job.expires_at` всегда не меньше соответствующего лимита, ссылка Turbotext гарантированно живёт 60 секунд, а остальные объекты не выходят за рамки собственного окна или дедлайна задачи.
- **Интеграция:**
  - Ingest API вызывает `deadline_guard.attach(job)` при создании задачи, использует `job.expires_at` в цикле ожидания и возвращает это поле в ответе клиенту.
...
### Краткие требования
- TTL ссылки фиксирован и составляет 60 секунд. Продление не предусмотрено: по истечении минуты ссылка и файл удаляются, связанные задачи получают `failure_reason = 'timeout'`.
...
* **Лимиты:**
  ...
  * TTL — ссылка истекает через 60 секунд после регистрации (`expires_at = now + MEDIA_PUBLIC_LINK_TTL_SEC`). Для файлов, связанных с активной ingest-задачей (`job_id` не `null`), воркер отслеживает наступление `T_sync_response`; если провайдер не скачал файл до истечения TTL, задача получает `failure_reason = 'timeout'`.
...
что провайдер скачал файл до истечения минуты; по достижении дедлайна задача получает `failure_reason = 'timeout'`, а временные данные
...
Необязательные поля можно опускать — сервер обновляет только переданные значения. Успешный ответ `200 OK` возвращает структуру, идентичную `GET /api/settings` (без раскрытия секретов). Раздел `media_cache` в ответе отражает только фиксированные значения TTL (`public_link_ttl_sec = 60`, `processed_media_ttl_hours = 72`) и служит для информирования UI: они определяются конфигурацией (`MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC`) и не меняются через API. При использовании дискового хранения в `Job.result_file_path` поле `processed_media_ttl_hours` задаёт срок хранения файла для диагностики; в текущем MVP результат очищается автоматически по истечении этого срока (72 часа).
...
3. Кнопка "Очистить мультимедиа кеш" запускает фоновую очистку временных объектов (`media_object` с истекшим TTL) и файлов, которые больше не упомянуты в `Job.result_file_path`; актуальные результаты остаются на месте, `template_media` не затрагиваются.
