...
1. Удаляет просроченные `media_object` и их файлы (`MEDIA_ROOT/tmp`), включая те, что потеряли связь с задачей, но игнорирует активные объекты с действующим TTL.
2. Проверяет `Result` и инициирует фоновый GC только для записей, чей `expires_at` уже наступил; свежие результаты с оставшимся TTL не затрагиваются.
3. Формирует отчёт об объёме удалённых временных файлов.

`template_media` и другие постоянные ресурсы очистка не затрагивает — они остаются в системе независимо от запуска кнопки.
...
После завершения фонового задания статистика очистки (количество удалённых файлов, объём) логируется и доступна через системные метрики; UI получает только факт планирования задания и не ожидает мгновенного удаления свежих результатов.
