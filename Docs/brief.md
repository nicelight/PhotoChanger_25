...
По получению запроса, в зависимости от того на какой входящий эндпоинт он пришел, платформа формирует исходящий запрос к соответствующей AI модели для обработки фотографии. Запрос формируется на основании данных из слота, к которому привязан входящий эндпоинт. Платформа получает тем или иным методом обработанную фотографию от AI модели. Платформа отправляет обработанную фотографию в теле ответа на пользовательский POST запрос. Исходная фотография хранится во временном хранилище только пока задача находится в статусах `pending`/`processing` и максимум `T_ingest_ttl` (≤ 60 секунд, не превышает `T_sync_response`): по наступлении дедлайна или финального статуса файл удаляется безусловно. Обработанные фотографии, отправленные пользователю в ответе на входящий POST запрос, удаляются спустя 3 дня с момента их сохранения во временной папке.
...
4. Фоновая обработка: worker(ы) забирают задачу из `asyncio.Queue` или из таблицы `job` (PG), вызывают внешнего AI‑провайдера и обновляют
...
5. Завершение задачи (финальный статус фиксируется только внутри окна `T_sync_response`):
   - `completed`: воркер получает результат до наступления дедлайна, сохраняет `Result` во временном хранилище с TTL 3 дня, фиксирует
     `Job.status = 'completed'` и передаёт обработанное изображение в API → API возвращает его клиенту (200 OK).
   - `failed_timeout`: как только истёк `T_sync_response`, API возвращает 504 Gateway Timeout, помечает задачу как `failed_timeout` и отправляет
     воркеру сигнал отмены. Воркер прекращает все действия по задаче, прерывает обращение к провайдеру (если возможно) и очищает временные
     данные без сохранения `Result`. Ретраи запрещены, DSLR Remote Pro должен инициировать новый ingest.
   - `failed_provider`: если провайдер вернул ошибку либо исчерпаны разрешённые ретраи до наступления дедлайна, воркер фиксирует
     `Job.status = 'failed_provider'`, освобождает ресурсы и возвращает ошибку в API (504/502 в зависимости от типа сбоя).
   - `cancelled`: внешняя отмена (ручное вмешательство, снятие слота из очереди) устанавливает `Job.status = 'cancelled'` и очищает
     временные данные.

6. Ретраи и back‑pressure: ограничение параллелизма на провайдера/слот; экспоненциальные ретраи при сетевых сбоях (до N попыток) выполняются
   только пока задача находится в статусе `processing` и до наступления дедлайна `T_sync_response`. Каждая попытка фиксируется в `Job.attempt`
   и логах. После фиксации любого финального состояния (`completed`, `failed_timeout`, `failed_provider`, `cancelled`) воркер немедленно
   прекращает обработку и освобождает ресурсы. Если дедлайн наступил до завершения успешной попытки, побочные процессы (webhook, polling,
   скачивание файлов) также отменяются.
Если результат от провайдера не пришёл в течение `T_sync_response`, ingest завершается 504 с фиксацией `failed_timeout` и без сохранения
`Result`; возобновление возможно только через новый POST от клиента.
```mermaid
sequenceDiagram
...
### Краткие требования
- TTL ссылки: по умолчанию 10–15 минут (настраиваемо). Значение должно покрывать дедлайн `T_sync_response`, но фактическое хранение
  ограничено временем жизни задачи: после финального статуса файлы удаляются немедленно.
- Форматы: JPEG/PNG/WEBP (минимальный набор можно расширять).
  Провайдеры, работающие по публичным ссылкам (например, Turbotext), обязаны ограничиваться этими MIME, чтобы избежать
  конвертации файлов перед отправкой.
...
* **Тело запроса:**
  ...
  * `ttl_sec` (`integer`, опционально) — желаемый срок жизни ссылки в секундах. Значение ограничено диапазоном `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC … media.max_manual_ttl_sec` (по умолчанию 900–86400). Если параметр не передан, используется `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC`.
...
* **Лимиты:**
  ...
  * TTL — ссылка истекает в `expires_at`, которое не превосходит `media.max_manual_ttl_sec` и не превышает срок жизни соответствующей задачи (`pending`/`processing`).
...
<binary jpeg data>
--boundary123
Content-Disposition: form-data; name="ttl_sec"
...
#### `POST /api/media/extend`

* **Назначение:** продлить `expires_at` для существующего временного файла, чтобы ссылка оставалась валидной во время обработки.
* **Аутентификация:** требуется `Authorization: Bearer <JWT>`; необходимо право `slots:write`.
* **Content-Type:** `application/json`.
* **Тело запроса:**
  ```json
  {
    "id": "<uuid>",          // обязательный идентификатор media_object
    "ttl_sec": 1800          // опционально: новый срок жизни в секундах
  }
  ```
  Если `ttl_sec` не задан, сервис использует `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC`. Новое `expires_at` не может превышать `media.max_manual_ttl_sec` и не может быть меньше текущего времени.
* **Успешный ответ (`200 OK`):**
  ```json
  {
    "id": "7f6c9d44-221a-4895-9f63-5e9f7137ba3f",
    "public_url": "https://api.example.com/public/media/7f6c9d44-221a-4895-9f63-5e9f7137ba3f",
    "expires_at": "2024-07-09T13:15:30Z"
  }
  ```
* **Ошибки:**
  * `400` — нарушены правила продления TTL (например, попытка сократить срок или запросить значение выше лимита).
  * `401`/`403` — неавторизованно или недостаточно прав.
  * `404` — указанный `media_object` не найден или уже очищен.
  * `409` — файл помечен на удаление из-за финального статуса задачи.
  * `429` — превышен лимит обращений к продлению.
  * `500`/`503` — системные ошибки или недоступность сервиса.

**Пример запроса:**

```http
POST /api/media/extend HTTP/1.1
Host: api.example.com
Authorization: Bearer <JWT>
Content-Type: application/json

{"id":"7f6c9d44-221a-4895-9f63-5e9f7137ba3f","ttl_sec":1800}
```
...
{"id":"7f6c9d44-221a-4895-9f63-5e9f7137ba3f","public_url":"https://api.example.com/public/media/7f6c9d44-221a-4895-9f63-5e9f7137ba3f","expires_at":"2024-07-09T13:15:30Z"}
```
...
- Пока задача в статусах `pending`/`processing`, воркер при необходимости продлевает TTL, чтобы покрыть `T_sync_response`.
- По завершении задачи со статусом `completed` файл остаётся доступным до `expires_at`; при `failed_timeout`, `failed_provider` или
  `cancelled` воркер помечает запись на немедленное удаление.
...
`Job.status` фиксирует жизненный цикл задачи с учётом таймаутов и повторных попыток. Как только API достигает дедлайна `T_sync_response`
и возвращает 504, запись переводится в `failed_timeout`, `locked_by` очищается, после чего задача считается финализированной и дополнительные
попытки не планируются. Воркеры, работающие через PostgreSQL, выбирают задачи запросом `SELECT … FOR UPDATE SKIP LOCKED`, обновляют
`locked_by`/`locked_at`, увеличивают `attempt` и записывают `last_attempt_at`. Для in-process `asyncio.Queue` таблица используется для
журналирования и восстановления после рестарта — запись создаётся синхронно с постановкой в очередь.
...
`media_object`, а поле `payload_path`/`storage_path` хранит относительный путь до итогового изображения. При таймауте API (шаг 5 архитектуры)
задача переводится в `failed_timeout`, воркер при получении такого статуса должен отменить активный запрос, удалить временные файлы
(`payload_path`, записи в `media_object` без `Result`) и прекратить дальнейшие вызовы. Ретраи (`attempt < max_attempts`) планируются через
`next_retry_at` только пока не наступил дедлайн `T_sync_response` и не зафиксирован финальный статус; если лимит попыток исчерпан до
наступления дедлайна, статус становится `failed_provider`, что отображается в статистике слота и в ответах API. После любой финализации
все связанные `media_object` без `Result` помечаются к немедленному удалению.
...
`ProcessingLog.status = 'TIMEOUT'` соответствует задачам с `Job.status = 'failed_timeout'`; `ERROR` агрегирует `failed_provider` и `cancelled`
...
1. **`configs/providers.json`** — статический каталог провайдеров и поддерживаемых операций. Файл публикуется веб-сервером как статический ресурс (например, `/static/providers.json`), поэтому UI загружает именно его для построения форм выбора и валидации параметров. Бэкенд использует тот же локальный файл при проверке запросов и формировании исходящих обращений к провайдерам. Флаг `requires_public_media` в конфигурации сигнализирует фронтенду, что для такого провайдера загрузчик должен предлагать только MIME из `MEDIA_ALLOWED_MIME` (по умолчанию JPEG/PNG/WEBP) и заранее блокировать HEIC/HEIF.
...
* **Тело запроса:** `multipart/form-data` с полями `file` (бинарный файл JPEG/PNG/WEBP/HEIC/HEIF), `slot_id` (TEXT), `setting_key`
  ...
  (по умолчанию — `image/jpeg`, `image/png`, `image/webp`), остальные форматы приводят к ответу `415 Unsupported Media Type`.
* **Успешный ответ (`201 Created`):**
  ```json
  {
...
- **Ошибки:**
  ...
  - `504 Gateway Timeout` — обработка не завершена в пределах синхронного окна ожидания (`Job.status = 'failed_timeout'`, воркер и очередь получают сигнал отмены).
