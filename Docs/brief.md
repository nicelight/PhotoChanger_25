...
5. Завершение задачи:
   - Успех (AI < 48 c): worker получает результат, сохраняет `Result` во временном хранилище с TTL 3 дня и передаёт обработанное
     изображение в API → API возвращает его клиенту (200 OK).
   - Таймаут (AI > 48 c): API возвращает 504 Gateway Timeout, закрывает соединение, помечает задачу как `failed_timeout` и
     ...
     временные данные без сохранения `Result`. DSLR Remote Pro должен отправить новый ingest-запрос с исходной фотографией.
     Для таймаутов сервер ведёт агрегированную статистику (счётчик, время, слот).

6. Ретраи и back‑pressure: ограничение параллелизма на провайдера/слот; экспоненциальные ретраи при сетевых сбоях (до N попыток); статус и счётчик попыток фиксируются в job/логах. Ретраи выполняются только до наступления финального состояния (`succeeded`, `failed_timeout`, `failed_error`); после фиксации финала воркер прекращает обработку и освобождает ресурсы.
...
**Таблица `Slot`**
*   `id`: `TEXT` (Primary Key, одна из 15 предустановленных статических ingest-ссылок)
...
> Для MVP доступно 15 слотов с фиксированными идентификаторами (`slot-001` … `slot-015`). Они создаются миграцией БД для
> каждого пользователя при инициализации сервиса и соответствуют заранее выписанным ingest-URL. Повторная настройка выполняется
> поверх той же записи: `id` остаётся неизменным, а привязанная ingest-ссылка сохраняется на всём сроке жизни пользователя.
> При сбросе настроек фронтенд помечает слот как свободный, но его `id` и ingest-URL продолжают принадлежать тому же
> пользователю.
...
Возвращает текущие глобальные настройки платформы. Требует авторизованного пользователя с правом `settings:read` (см. UI «Страница настроек»). Ответ 200 OK содержит JSON следующей структуры:
...
Кнопка на странице настроек вызывает `POST /api/media/cache/purge`. Эндпоинт требует права `settings:write`, выполняет фоновое удаление всех записей `media_object`, у которых `expires_at` не наступил, но которые относятся к «обработанным фотографиям», сохраняемым до 3 суток, и удаляет связанные файлы в `MEDIA_ROOT`. Ответ: `202 Accepted` с телом
...
* **Назначение:** Получить список всех 15 преднастроенных слотов текущего пользователя и их актуальные настройки.
* **Аутентификация:** требуется `Authorization: Bearer <JWT>`.
...
- **Авторизация:** глобальный пароль, передаваемый в теле (поле `password`). Сравнивается с хешем, сохранённым для данного слота.
- **slotId:** статический идентификатор ingest-слота (предзаданные значения `slot-001` … `slot-040`).

#### Тело запроса
- **Тип:** `multipart/form-data`.
- **Обязательные поля:**
  - `password` — строка. Проверяется на совпадение с хешем, связанным со слотом; требуется непустое значение.
...
- **Ошибки:**
  ...
  - `401 Unauthorized` — пароль не совпал с ожидаемым для слота.
