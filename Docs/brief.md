...
По получению запроса, в зависимости от того на какой входящий эндпоинт он пришел, платформа формирует исходящий запрос к соответствующей AI модели для обработки фотографии. Запрос формируется на основании данных из слота, к которому привязан входящий эндпоинт. Платформа получает тем или иным методом обработанную фотографию от AI модели. Платформа отправляет обработанную фотографию в теле ответа на пользовательский POST запрос. Исходная фотография хранится во временном хранилище только пока задача находится в статусах `pending`/`processing` и максимум `T_ingest_ttl` (≤ 50 секунд, не превышает `T_sync_response`): по наступлении дедлайна или финального статуса файл удаляется безусловно. Продление временных ссылок воркерами не требуется: базовый TTL `media_object` подбирается так, чтобы покрывать пользовательское окно ожидания без дополнительных запросов к API. Обработанные фотографии, отправленные пользователю в ответе на входящий POST запрос, сохраняются в `Result` до истечения 3 дней (`media.processed_ttl_hours = 72`) и затем удаляются плановым фоновым процессом; административные действия (включая `POST /api/media/cache/purge`) не сокращают гарантированный срок хранения итогового изображения.
...
5. Завершение задачи (финальный статус фиксируется только внутри окна `T_sync_response`):
   - `completed`: воркер получает результат до наступления дедлайна, сохраняет `Result` во временном хранилище с TTL 3 дня, фиксирует
     `Job.status = 'completed'` и передаёт обработанное изображение в API → API возвращает его клиенту (200 OK). После фиксации
     успеха воркер очищает только исходный DSLR-файл (`payload_path`/`source_media_id`), оставляя запись `Result` и лежащий на диске
     итоговый файл нетронутыми до истечения их TTL.
  - `failed_timeout`: как только истёк `T_sync_response`, API возвращает 504 Gateway Timeout, помечает задачу как `failed_timeout` и отправляет
    ...
    данные без сохранения `Result`. Все текущие попытки прерываются, новые не запускаются; после фиксации статуса ретраи для этой задачи
    запрещены, DSLR Remote Pro должен инициировать новый ingest.
...
6. Ретраи и back‑pressure: ограничение параллелизма на провайдера/слот; экспоненциальные ретраи при сетевых сбоях (до N попыток) выполняются
   ...
   скачивание файлов) также отменяются; статус `failed_timeout` означает окончательную отмену текущей задачи, и повтор возможен только
   через новый POST от клиента.
Продление временных ссылок (`media_object`) допускается только пока задача находится в статусах `pending`/`processing` и может инициироваться
лишь интерактивными клиентами (UI, интеграции). Воркерам и фоновым процессам запрещено продлевать TTL автоматически, особенно после фиксации
любого финального статуса, включая `failed_timeout`.
...
### Краткие требования
- TTL ссылки: базовое значение ≤ 50 секунд (совпадает с `T_ingest_ttl` и не превосходит `T_sync_response`). После истечения базового срока ссылка безусловно удаляется; дальнейшее продление возможно только ручным вызовом `/api/media/extend` и не используется воркерами.
...
* MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC — время жизни ссылок (по умолчанию 50)
...
* **Тело запроса:**
  ...
  * `ttl_sec` (`integer`, опционально) — желаемый срок жизни ссылки в секундах. Значение ограничено диапазоном `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC … media.max_manual_ttl_sec` (по умолчанию 50–86400). Если параметр не передан, используется `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC`.
...
* **Лимиты:**
  ...
  * TTL — ссылка истекает в `expires_at`, которое не превосходит `media.max_manual_ttl_sec`, не превышает срок жизни соответствующей задачи (`pending`/`processing`) и по умолчанию ограничено `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC` (≤ 50 с). Для файлов, связанных с активной ingest-задачей (`job_id` не `null`), итоговое значение дополнительно ограничивается `T_sync_response` (≤ 50 с), чтобы гарантировать консистентность с окнами ожидания API.
...
* **Назначение:** опционально продлить `expires_at` для существующего временного файла, если нужно вручную сохранить ссылку дольше стандартного пользовательского окна ожидания.
* **Аутентификация:** требуется `Authorization: Bearer <JWT>`; необходимо право `slots:write`.
* **Content-Type:** `application/json`.
* **Тело запроса:**
  ```json
...
* **Правила использования:**
  * Продление разрешено только до тех пор, пока связанная задача находится в статусах `pending`/`processing`; при попытке продлить ссылку после фиксации любого финального статуса (`completed`, `failed_timeout`, `failed_provider`, `cancelled`) сервис возвращает `409 Conflict` и помечает файл к удалению по базовому TTL.
  * Инициировать продление могут только интерактивные клиенты (UI или внешние интеграции). Воркерам и фоновым процессам запрещено автоматическое продление, правила остановки задач по наступлении `T_sync_response` и очистка данных после таймаута остаются неизменными.
...
* **Ошибки:**
  ...
  * `409` — файл помечен на удаление из-за финального статуса задачи; сервис подчёркивает, что после наступления `T_sync_response`/`failed_timeout` продление невозможно.
...
- Пока задача в статусах `pending`/`processing`, временный файл живёт до истечения базового `expires_at`, которое не превышает `T_sync_response` (≤ 50 с) и не продлевается автоматически.
...
- Turbotext обязан скачать переданные `url`/`url_image_target` сразу после регистрации файла: по истечении базового TTL (≤ 50 с) ссылка истекает без продления со стороны платформы.
...
`external_queue_id` и `external_async_id` обновляются атомарно: воркер записывает оба значения в рамках одной транзакции до начала основного вызова провайдера. Потеря идентификаторов запрещена — при ошибке обновления задача помечается как `failed_provider`, а ресурсы очищаются.
...
`Result.expires_at` заполняется воркером исходя из глобальной настройки `media.processed_ttl_hours` (по умолчанию 72 часа). При
успешном завершении обработки воркер атомарно обновляет `Job.status = 'completed'`, создаёт запись в `Result` и кладёт
обработанный файл в `MEDIA_ROOT`. Реестр `Result` используется для учёта сохранённых файлов и контроля срока их жизни: внутренние
сервисы платформы сверяются с `expires_at`, чтобы определить доступность результата. Плановая очистка (GC) удаляет просроченные
записи и освобождает связанные файлы, опираясь на `Result.expires_at`. Отдельный фоновый джоб `result_gc` запускается по расписанию
раз в 72 часа и для каждого `Result`, у которого `expires_at < now()`, удаляет запись и связанный файл из `MEDIA_ROOT/results`
(а также синхронно чистит дублирующий `media_object`, если он существует). Если требуется предоставить итоговое изображение по
публичной ссылке (например, для скачивания из UI), воркер дополнительно создаёт запись в `media_object` с тем же `storage_path`
и выставляет `expires_at`, совпадающий с `Result.expires_at`; таким образом, TTL финального результата всегда равен
`media.processed_ttl_hours`, независимо от наличия публичной ссылки.
...
просроченные файлы. Продление через `POST /api/media/extend` допускается только вручную и не используется воркерами либо
фоновыми задачами.

Продление TTL выполняется только вручную:

* **UI/бекенд при сохранении слота.** Когда администратор сохраняет настройки или запускает тестовую загрузку, фронтенд
  передаёт текущие `media_object.id` в теле запроса. Если нужно подготовить предпросмотр дольше стандартного окна (≤ 50 с),
  оператор вручную инициирует `POST /api/media/extend` для конкретного вложения. Запрос ограничивает новое значение `expires_at`
  настройкой `media.max_manual_ttl_sec`, а для файлов с заполненным `job_id` дополнительно применяет ограничение `T_sync_response`
  (≤ 50 с), чтобы ручные продления не нарушали таймауты ingest.
* **Воркеры:** автоматическое продление `media_object` перед постановкой задачи в очередь не выполняется. Базовый TTL обеспечивает
  доступность ссылки только в рамках пользовательского ожидания; после истечения срока запись автоматически истекает и очищается
  без участия воркера.
...
> **Примечание о разграничении `Result` и `media_object`:**
>
> | Сценарий | Где хранится файл | TTL | Нужно ли создавать `media_object` |
> | --- | --- | --- | --- |
> | Итоговое изображение, выдаваемое клиенту или отображаемое в UI | Запись в `Result` + файл в `MEDIA_ROOT/results` | `media.processed_ttl_hours` (72 ч по умолчанию) → значение попадает и в `Result.expires_at`, и в связанный `media_object` (если требуется публичная ссылка) | Только если нужен публичный URL; при хранении результата «для выдачи по API» запись `media_object` не обязательна |
> | Временная ссылка для провайдера (Turbotext и т. п.) или тестовых загрузок | Запись в `media_object` + файл в `MEDIA_ROOT/tmp` | Задаётся при регистрации файла, ограничено `media.public_link_default_ttl_sec`/`media.max_manual_ttl_sec`; максимум покрывает `T_sync_response` | Да, это основной механизм получения публичного URL |
...
>
> Таким образом, финальные результаты всегда учитываются сущностью `Result`, а `media_object` либо обслуживает короткоживущие ссылки, либо дублирует `Result` для публикации файла без изменения его срока жизни.
...
```json
{
  ...
  "media_cache": {
    ...
    "public_link_default_ttl_sec": 50,
...
Необязательные поля можно опускать — сервер обновляет только переданные значения. Успешный ответ `200 OK` возвращает структуру, идентичную `GET /api/settings` (без раскрытия секретов). Валидация: `processed_media_ttl_hours` ∈ [1, 168], `public_link_default_ttl_sec` ∈ [50, 86400], `max_manual_ttl_sec` ≥ `public_link_default_ttl_sec`. Значения, превышающие `T_sync_response`, применимы только к объектам без `job_id` (например, предпросмотрам до запуска ingest); для файлов, связанных с задачами, сервис автоматически ограничит TTL значением `T_sync_response`.
...
* `full` (по умолчанию) — планирует немедленный запуск той же процедуры, что и плановый джоб `result_gc`: в очередь ставится очистка истёкших `Result` и связанных с ними файлов, а также синхронное удаление просроченных `media_object`.
* `media_objects` — ограничивает выполнение очисткой только временных `media_object` (папка `MEDIA_ROOT/tmp`), оставляя реестр `Result` без изменений.

В обоих режимах активные результаты и файлы с непросроченным TTL остаются нетронутыми: эндпоинт никогда не удаляет `Result`, у которых `expires_at ≥ now()`, даже если запрошен полный режим.
...
`job_id` — идентификатор фонового задания, `expires_cutoff` — момент времени, до которого будут очищены сущности (обычно `now()` на момент постановки). После завершения фонового задания в системные метрики отправляется счётчик `purged_entities` с разбивкой по типам (`results`, `media_objects`) и объёму в байтах. UI получает только факт постановки джоба и может периодически опрашивать мониторинг при необходимости.
...
* Значения TTL (`processed_media_ttl_hours`, `public_link_default_ttl_sec`, `max_manual_ttl_sec`) и другие не чувствительные параметры хранятся в `app_settings` с ключами `media.processed_ttl_hours`, `media.public_link_default_ttl_sec`, `media.max_manual_ttl_sec` и аналогичными. Для таких записей `is_secret = false`, а `value_json` содержит примитив (`integer`, `string`) или объект с описанием параметра. Эти настройки распространяются на `Result` (через `processed_media_ttl_hours`) и на `media_object`; `template_media` хранится без TTL и очищается вручную. Миграции должны обеспечить начальные значения (72, 50 и 86400 соответственно) и поддерживать уникальность ключей. Для объектов, связанных с ingest-задачами, прикладной код обязан обрезать TTL по `T_sync_response`, чтобы даже при повышении `media.public_link_default_ttl_sec` для предпросмотров не нарушались таймауты ingest. Если для удобства UI требуется отображать метаданные провайдеров без расшифровки (`project_id`, `region` и т. п.), используются отдельные записи `provider.<id>.<field>` с `is_secret = false`.
...
`source_media_id` хранит идентификатор временной записи в `media_object`; базовый TTL покрывает только пользовательское окно ожидания (≤ 50 с). При необходимости подготовить предпросмотр дольше этого периода оператор вручную вызывает `POST /api/media/extend` для конкретного файла, иначе ссылка истечёт перед запуском ingest. Общие свойства
...
3. Перед сохранением слота бекенд формирует объект по схеме и сериализует JSON в `Slot.settings_json`. Если нужно сохранить конкретный `media_object.id` дольше базового TTL, оператор вручную инициирует `POST /api/media/extend` до сохранения.
...
```
Ответ при успехе: бинарный `image/jpeg`|`image/png`|`image/webp`|`image/heic`|`image/heif` (синхронно, short-poll ≤ 50 с; см. пункт&nbsp;5).
...
3. Кнопка "Очистить мультимедиа кеш" запускает фоновую очистку только временных объектов (`media_object` с истекшим TTL) и, при необходимости, плановый GC устаревших `Result`; свежие результаты с оставшимся сроком хранения не удаляются, `template_media` не затрагиваются.
   * В интерфейсе рядом с кнопкой показан серый хинт: «Итоговые изображения удаляются автоматически раз в 3 дня, активные результаты останутся доступными».
