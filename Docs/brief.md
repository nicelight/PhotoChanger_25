...
**Примечание:** воркеры и фоновые процессы больше не вызывают `/api/media/extend` автоматически. Временное медиа живёт ровно базовый TTL (`T_ingest_ttl` ≤ 60 с), а интеграции (например, Turbotext) обязаны скачивать файл сразу после регистрации.
