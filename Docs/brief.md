...
записи и освобождает связанные файлы, опираясь на `Result.expires_at`. Если требуется предоставить итоговое изображение по
...
Кнопка на странице настроек вызывает `POST /api/media/cache/purge`. Эндпоинт требует права `settings:write` (см. раздел «Права доступа и роли») и планирует фоновый джоб, который:

1. Находит все записи `Result`, чей `expires_at` ещё не наступил, удаляет соответствующие файлы из `MEDIA_ROOT/results` и помечает записи как очищенные.
2. Удаляет связанные с этими задачами `media_object` (если они создавались для публикации финального файла) и их файлы.
3. После этого очищает оставшиеся активные `media_object`, которые используются только как временные публичные ссылки (`MEDIA_ROOT/tmp`), независимо от их текущего TTL.

`template_media` и их привязки очистка не затрагивает — постоянные шаблоны остаются в системе. Ответ: `202 Accepted` с телом
