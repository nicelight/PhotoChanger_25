...
По получению запроса, в зависимости от того на какой входящий эндпоинт он пришел, платформа формирует исходящий запрос к соответствующей AI модели для обработки фотографии. Запрос формируется на основании данных из слота, к которому привязан входящий эндпоинт. Платформа получает тем или иным методом обработанную фотографию от AI модели. Платформа отправляет обработанную фотографию в теле ответа на пользовательский POST запрос. Исходная фотография хранится во временном хранилище только пока задача находится в статусах `pending`/`processing` и максимум `T_ingest_ttl` (≤ 50 секунд, не превышает `T_sync_response`): по наступлении дедлайна или финального статуса файл удаляется безусловно. Временные публичные ссылки `media_object` живут 60 секунд и не продлеваются: если провайдер не скачал файл за это время, задача завершается `failed_timeout`, а временные данные удаляются. Обработанные фотографии, отправленные пользователю в ответе на входящий POST запрос, сохраняются в `Result` до истечения 3 дней (`media.processed_ttl_hours = 72`) и затем удаляются плановым фоновым процессом; административные действия (включая `POST /api/media/cache/purge`) не сокращают гарантированный срок хранения итогового изображения.
...
Временные ссылки (`media_object`) живут фиксированное время — 60 секунд. Если за это окно провайдер не скачал файл, задача считается невыполненной: воркер помечает её как `failed_timeout`, временные данные удаляются, и повторный запуск возможен только через новый POST.
...
### Краткие требования
- TTL ссылки фиксирован и составляет 60 секунд. Продление не предусмотрено: по истечении минуты ссылка и файл удаляются, связанные задачи помечаются `failed_timeout`.
...
* MEDIA_PUBLIC_LINK_TTL_SEC — фиксированное время жизни ссылок (60 секунд)
...
* **Лимиты:**
  ...
  * TTL — ссылка истекает через 60 секунд после регистрации (`expires_at = now + MEDIA_PUBLIC_LINK_TTL_SEC`). Для файлов, связанных с активной ingest-задачей (`job_id` не `null`), воркер отслеживает наступление `T_sync_response`; если провайдер не скачал файл до истечения TTL, задача фиксируется как `failed_timeout`.
...
просроченные файлы. Значение фиксировано и равно моменту регистрации + 60 секунд.
...
что провайдер скачал файл до истечения минуты; по достижении дедлайна задача фиксируется как `failed_timeout` и временные данные
...
> | Временная ссылка для провайдера (Turbotext и т. п.) или тестовых загрузок | Запись в `media_object` + файл в `MEDIA_ROOT/tmp` | `MEDIA_PUBLIC_LINK_TTL_SEC` (60 с); по истечении срока ссылка удаляется без продления | Да, это основной механизм получения публичного URL |
...
```json
{
  ...
  "media_cache": { "max_manual_ttl_sec": 14400 }
}
```

Необязательные поля можно опускать — сервер обновляет только переданные значения. Успешный ответ `200 OK` возвращает структуру, идентичную `GET /api/settings` (без раскрытия секретов). Раздел `media_cache` включает фиксированные TTL обработанных результатов (`processed_media_ttl_hours = 72`) и временных ссылок (`public_link_ttl_sec = 60`), а также настройку `max_manual_ttl_sec`, определяющую верхний предел для TTL, который оператор может задать вручную при очистке медиакеша. Значение `max_manual_ttl_sec` валидируется на диапазон 60…86400 секунд; попытки выйти за пределы приводят к ошибке `422 Unprocessable Entity`.
...
* Значения TTL (`processed_media_ttl_hours`, `public_link_ttl_sec`) и другие нечувствительные параметры хранятся в `app_settings` с ключами `media.processed_ttl_hours` и `media.public_link_ttl_sec`. Для таких записей `is_secret = false`, а `value_json` содержит примитив (`integer`) или объект с описанием параметра. Эти настройки распространяются на `Result` (через `processed_media_ttl_hours`) и на `media_object`; `template_media` хранится без TTL и очищается вручную. Миграции должны обеспечить начальные значения (72 и 60 соответственно) и поддерживать уникальность ключей. Значение `media.public_link_ttl_sec` фиксировано и не изменяется через административные запросы. Если для удобства UI требуется отображать метаданные провайдеров без расшифровки (`project_id`, `region` и т. п.), используются отдельные записи `provider.<id>.<field>` с `is_secret = false`.
...
3. Перед сохранением слота бекенд формирует объект по схеме и сериализует JSON в `Slot.settings_json`. Если требуется оперировать `media_object.id` позже стандартного окна в 60 секунд, файл необходимо загрузить повторно ближе к запуску ingest; продление ссылок не поддерживается.
