...
5. Результат:
   ...
   - Таймаут (AI > 48 c): API возвращает 504 Gateway Timeout, закрывает соединение и помечает задачу как `failed_timeout`.
     Worker должен прекратить дальнейшие действия по данной задаче: отменить обращение к провайдеру (если возможно) либо
     безопасно очистить промежуточные данные. Никакой дополнительной выдачи результата или повторной доставки по этому запросу
     не производится; DSLR Remote Pro должен отправить новый ingest-запрос с исходной фотографией. Для таймаутов сервер ведёт
     агрегированную статистику (счётчик, время, слот).
...
Этот эндпоинт предназначен для приёма `POST` запросов от программы DSLR Remote Pro. Будет сгенерировано 40 статических коротких ingest-ссылок, которые можно привязывать к слотам.
