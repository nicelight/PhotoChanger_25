...
формы (`url`, `url_image_target`, `content`, `face_restore` и т. д.) значениям из настроек слота и выбранному источнику медиа. Общие свойства
...
1. Загруженные шаблоны отправляются на `POST /api/template-media/register`, который возвращает `template_media_id` и обновляет
   `slot_template_binding`.
2. Временные вложения (тестовые рендеры, предпросмотры) регистрируются через `POST /api/media/register`, что возвращает `media_object.id`.
3. Бэкенд формирует объект по схеме, подставляя соответствующие идентификаторы и сериализует JSON в `Slot.settings_json`.
