...
- **Единый дедлайн Job:** при создании записи `Job` фиксируется поле `expires_at = created_at + T_sync_response`. Значение хранится в БД и не меняется; API, воркеры и механизмы очистки получают дедлайн только через `DeadlineGuard`.
- **Политика TTL артефактов:** для всех временных объектов применяется формула `artifact_expires_at = min(job.expires_at, created_at + T_media_limit)`. Так исходный `payload` не выходит за пределы окна синхронного ответа, а публичные ссылки Turbotext удаляются по наступлении дедлайна задачи, даже если их собственный лимит (60 секунд) ещё не истёк.
- **Интеграция:**
