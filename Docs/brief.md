...
Итоговое изображение хранится непосредственно в полях `result_*` текущей записи `Job`. Фоновая очистка результатов не требуется — сохраняется только последний успешный ответ; при запуске новой задачи содержимое перезаписывается. Если требуется предоставить итоговое изображение по публичной ссылке (например, для скачивания из UI), воркер дополнительно создаёт запись в `media_object`, но срок жизни финального результата определяется фактом последнего успешного запуска, а не отдельным TTL.
...
> | --- | --- | --- | --- |
> | Итоговое изображение, выдаваемое клиенту или отображаемое в UI | Поля `Job.result_*` (`result_inline_base64` или `result_file_path` в `MEDIA_ROOT/results`) | Живёт до появления нового успешного запуска (отдельный TTL не применяется) | Только если нужен публичный URL; при хранении результата для API запись `media_object` не обязательна |
...
Необязательные поля можно опускать — сервер обновляет только переданные значения. Успешный ответ `200 OK` возвращает структуру, идентичную `GET /api/settings` (без раскрытия секретов). Раздел `media_cache` включает настройки временных ссылок (`public_link_ttl_sec = 60`) и параметры ручной очистки. Поле `processed_media_ttl_hours` сохраняется для совместимости: при использовании дискового хранения в `Job.result_file_path` оно определяет, как долго сохранять файл для диагностики; в текущем MVP результат хранится как последний ответ без гарантированного TTL. Параметр `max_manual_ttl_sec` задаёт верхний предел для TTL, который оператор может указать при очистке медиакеша (валидация диапазона 60…86400 секунд, при выходе за пределы — ошибка `422 Unprocessable Entity`).
