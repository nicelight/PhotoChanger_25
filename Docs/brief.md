...
### Turbotext
- Выполняет style-transfer, image-to-image и deepfake-сценарии через `api_ai` с поддержкой очередей и webhook.
- Требует временных публичных ссылок (`media_object`) и работает с MIME `image/jpeg`, `image/png`, `image/webp`.
- Возвращает `queueid`/`asyncid`, которые сохраняются в `Job.provider_job_reference` для polling или webhook-процессинга.
...
- **Воркеры** — исполняют операции провайдера, отслеживают `expires_at`, поддерживают polling/webhook и гарантируют очистку временных данных.
- **UI и админ-панель** — управляют слотами, шаблонами и глобальными настройками, опираясь на статическую конфигурацию провайдеров (`configs/providers.json`).
- **Мониторинг и логирование** — фиксируют статусы ingest, таймауты, ретраи и операции очистки в `ProcessingLog` и связанных метриках.
...
- Для асинхронных сценариев Turbotext сохраняется `queueid`/`asyncid`; воркер либо опрашивает `do=get_result`, либо принимает webhook. Если результат приходит после дедлайна,
...
- [spec/docs/blueprints/use-cases.md](../spec/docs/blueprints/use-cases.md) — сценарии UI и ingest, включая таймауты и webhook.
