...
создаёт `Job`, рассчитывает дедлайны (`expires_at`, `T_ingest_ttl`, `T_public_link_ttl`, `T_result_retention`) и ожидает ответ воркера до наступления `T_sync_response`.
