...
>
> Таким образом, финальные результаты всегда учитываются сущностью `Result`, а `media_object` либо обслуживает короткоживущие
> ссылки, либо дублирует `Result` для публикации файла без изменения его срока жизни.
