...
Платформа использует архитектуру **"Асинхронного моста"**, чтобы обрабатывать запросы от `DSLR Remote Pro` в рамках  50-секундного таймаута, даже если AI-модель работает дольше. Параметр 50 секунд можно будет задавать на странице статистики

1.  **Прием запроса:** `Ingest API` получает `POST` запрос.
2.  **Постановка в очередь:** Запрос немедленно помещается в быструю очередь задач (например, `Redis`).
3.  **Ожидание с таймаутом:** API-обработчик начинает ожидать результат, но с внутренним таймаутом ~48 секунд.
4.  **Фоновая обработка:** Отдельный сервис (`Worker`) забирает задачу из очереди и отправляет ее в AI-модель.
5.  **Результат:**
    *   **Успех (AI < 48с):** `Worker` получает результат, API-обработчик отправляет его клиенту.
    *   **Таймаут (AI > 48с):** API-обработчик возвращает ошибку клиенту. `Worker` может продолжать обработку, но результат уже не будет доставлен в этом запросе.
...
sequenceDiagram
    ...
    AI_Provider-->>-Worker: Готовое изображение
    Worker->>API: Уведомить о результате
...
Для хранения состояния приложения, настроек слотов и сбора статистики будет использоваться база данных SQLite.
...
*   `last_reset_at`: `DATETIME` (Дата последнего сброса статистики для этого слота)

**Таблица `ProcessingLog`**
*   `id`: `INTEGER` (Primary Key)
*   `slot_id`: `TEXT` (Foreign Key -> Slot)
*   `created_at`: `DATETIME` (Время получения ingest-запроса)
