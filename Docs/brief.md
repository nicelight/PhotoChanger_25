...
попытки не планируются. Воркеры выбирают задачи запросом `SELECT … FOR UPDATE SKIP LOCKED`, обновляют `locked_by`/`locked_at`, увеличивают
`attempt` и фиксируют `last_attempt_at`. Очередь в PostgreSQL — единый механизм для постановки, восстановления после рестартов и контроля
back-pressure; дополнительных in-process структур не используется.
