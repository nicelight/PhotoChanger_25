...
### Краткие требования
- TTL ссылки: базовое значение ≤ 60 секунд (совпадает с `T_ingest_ttl` и не превосходит `T_sync_response`). После истечения базового срока ссылка безусловно удаляется; дальнейшее продление возможно только ручным вызовом `/api/media/extend` и не используется воркерами.
...
* **Тело запроса:**
  ...
  * `ttl_sec` (`integer`, опционально) — желаемый срок жизни ссылки в секундах. Значение ограничено диапазоном `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC … media.max_manual_ttl_sec` (по умолчанию 60–86400). Если параметр не передан, используется `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC`.
...
* **Лимиты:**
  ...
  * TTL — ссылка истекает в `expires_at`, которое не превосходит `media.max_manual_ttl_sec`, не превышает срок жизни соответствующей задачи (`pending`/`processing`) и по умолчанию ограничено `MEDIA_PUBLIC_LINK_DEFAULT_TTL_SEC` (≤ 60 с).
...
- Пока задача в статусах `pending`/`processing`, временный файл живёт до истечения базового `expires_at`, которое не превышает `T_sync_response` (≤ 60 с) и не продлевается автоматически.
...
- Turbotext обязан скачать переданные `url`/`url_image_target` сразу после регистрации файла: по истечении базового TTL (≤ 60 с) ссылка истекает без продления со стороны платформы.
...
* **UI/бекенд при сохранении слота.** Когда администратор сохраняет настройки или запускает тестовую загрузку, фронтенд
  передаёт текущие `media_object.id` в теле запроса. Если нужно подготовить предпросмотр дольше стандартного окна (≤ 60 с),
...
```json
{
  ...
  "media_cache": {
    ...
    "public_link_default_ttl_sec": 60,
