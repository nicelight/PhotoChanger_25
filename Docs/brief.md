...
с 504, фиксируя статус задачи. То же значение используется как единый TTL для всех временных артефактов; подробности раскрыты в [доменной модели](../spec/docs/blueprints/domain-model.md).
...
Если провайдер Turbotext не возвращает результат до наступления `T_sync_response`, Ingest API отвечает 504 и немедленно завершает задачу: воркер не выполняет дальнейший polling, связанные временные ресурсы очищаются, публичные ссылки инвалидируются.
...
- **Воркеры** — исполняют операции провайдера, отслеживают `expires_at = created_at + T_sync_response`, не выходят за пределы общего дедлайна и гарантируют очистку временных данных.
