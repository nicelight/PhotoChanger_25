...
* **Назначение:** Получить список слотов текущего пользователя.
* **Аутентификация:** требуется `Authorization: Bearer <JWT>`.
* **Пагинация:** query-параметры `page` (>=1, по умолчанию 1) и `page_size` (1–100, по умолчанию 20).
* **Сортировка:** `sort_by` (`name`, `created_at`, `last_reset_at`; дефолт `created_at`) и `sort_order` (`asc`/`desc`, дефолт `asc`).
* **Фильтрация:**
  * `provider_id` — по идентификатору провайдера;
  * `operation_id` — по идентификатору операции;
  * `search` — подстрочный поиск по имени слота.
* **Успешный ответ (`200 OK`):**
  ```json
  {
    "data": [
  ...
  ```
* **Ошибки:** `400` (некорректные параметры), стандартные ошибки авторизации/доступа.

**`POST /api/slots`**

* **Назначение:** Создать новый слот (привязка настроек к одной из свободных ingest-ссылок).
* **Аутентификация:** требуется `Authorization: Bearer <JWT>`.
* **Тело запроса:**
  ```json
  {
...
* **Успешный ответ (`201 Created`):**
  ...
  ```
* **Ошибки:** `400`/`422` (ошибка валидации), `409` (слот уже занят), `401`/`403`, `500`.
...
* **Назначение:** Обновить настройки слота.
* **Аутентификация:** требуется `Authorization: Bearer <JWT>`.
* **Тело запроса:**
  ```json
  {
...
* **Успешный ответ (`200 OK`):**
  ...
  ```
* **Ошибки:** `400`/`422` (ошибка валидации), `404` (слот не найден), `409` (конфликт версий), стандартные ошибки авторизации.
