...
По получению запроса, в зависимости от того на какой входящий эндпоинт он пришел, платформа формирует исходящий запрос к соответствующей AI модели для обработки фотографии. Запрос формируется на основании данных из слота, к которому привязан входящий эндпоинт. Платформа получает тем или иным методом обработанную фотографию от AI модели. Платформа отправляет обработанную фотографию в теле ответа на пользовательский POST запрос. Исходная фотография хранится во временном хранилище только пока задача находится в статусах `pending`/`processing` и максимум `T_ingest_ttl` (≤ 60 секунд, не превышает `T_sync_response`): по наступлении дедлайна или финального статуса файл удаляется безусловно. Обработанные фотографии, отправленные пользователю в ответе на входящий POST запрос, сохраняются в `Result` до истечения 3 дней (`media.processed_ttl_hours = 72`) и затем удаляются плановым фоновым процессом; административные действия (включая `POST /api/media/cache/purge`) не сокращают гарантированный срок хранения итогового изображения.
