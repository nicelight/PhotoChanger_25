...
Платформа использует архитектуру "Асинхронного моста" для обработки запросов от DSLR Remote Pro в рамках 50‑секундного таймаута, даже если AI‑модель работает дольше. Значение таймаута (`T_sync_response`, по умолчанию 48 с, допустимый диапазон 45–60 с) настраивается на странице статистики и управляет временными TTL, живущими не дольше минуты.
