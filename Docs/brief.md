...
5. Финализация задачи (фиксируется только внутри окна `T_sync_response`):
   - Успех: воркер успевает получить результат, заполняет поля `Job.result_*`, выставляет `is_finalized = true`, `failure_reason = null`
     и возвращает изображение в API → клиент получает 200 OK. Base64-строка хранится в `Job.result_inline_base64` только на время ожидания ответа и обнуляется сразу после отправки HTTP 200. После этого очищаются только исходные файлы (`payload_path`/`source_media_id`).
   - Таймаут: при достижении `T_sync_response` API возвращает 504 Gateway Timeout, выставляет `is_finalized = true` и `failure_reason = 'timeout'`
     и отправляет воркеру сигнал отмены. Воркер прекращает обращения к провайдеру, удаляет временные данные, очищает `result_inline_base64`, поле `result` остаётся пустым.
...
### Контроль дедлайнов и TTL

Единый дедлайн задачи и связанные TTL вычисляются прямо в момент создания `Job`, отдельный сервис для синхронизации не используется.
...
- `T_public_link_ttl` — `app_settings["media.public_link_ttl_sec"] = clamp(T_sync_response, 45, 60)`; жёсткий лимит времени жизни публичных ссылок Turbotext и других временных артефактов.
...
- **Единый дедлайн Job:** при создании записи `Job` фиксируется поле `expires_at = created_at + T_job_deadline`, где `T_job_deadline = max(T_sync_response, T_public_link_ttl)`. Значение хранится в БД и не меняется; API, воркеры и механизмы очистки читают его напрямую из записи `Job` и пересчитывают остаток времени через общие хелперы доменного слоя.
...
- **Интеграция:**
  - Ingest API рассчитывает `job.expires_at` сразу при создании записи и использует его в цикле ожидания; ответ остаётся бинарным/общей ошибкой согласно контракту ingest и не содержит отдельных полей дедлайна.
  - Воркеры получают `job.expires_at` вместе с записью и перед каждой попыткой взаимодействия с провайдером проверяют оставшееся время через хелперы доменного слоя; отмена запускает очистку временных данных по тем же формулам TTL.
  - Очистка временных файлов и публичных ссылок опирается на `job.expires_at` и значения TTL из конфигурации; периодические задачи берут параметры напрямую, без отдельного сервиса.
...
*   `result_file_path`: `TEXT` (nullable, относительный путь к итоговому файлу в `MEDIA_ROOT/results`)
...
> | --- | --- | --- | --- |
> | Итоговое изображение, выдаваемое клиенту или отображаемое в UI | Поля `Job.result_*` (файл в `MEDIA_ROOT/results` по `result_file_path`) | `processed_media_ttl_hours` (72 часа) либо перезапись при новом успешном запуске | Только если нужен публичный URL; при хранении результата для API запись `media_object` не обязательна |
...
* Таймаут синхронного ожидания хранится в `app_settings["ingest.sync_response_timeout_sec"]`. После обновления значения прикладной код пересчитывает производные TTL (`media.ingest_ttl_sec`, `media.public_link_ttl_sec`) и формулу `T_job_deadline = max(T_sync_response, T_public_link_ttl)` на лету, чтобы новые задачи сразу получали корректный дедлайн.
...
* **Назначение:** показать очередь и завершённые задачи с агрегированными дедлайнами и метриками, рассчитываемыми по сохранённому в `Job` дедлайну.
...
* `deadline` — агрегат (`expires_at`, `remaining_ms`, `is_expired`), отражающий единый дедлайн `Job.expires_at` и остаток времени, вычисленный напрямую из временных меток.
...
* **Особенности:** маршрут read-only, использует те же дедлайны, что сохранены в `Job`, и не заменяет ingest API — изменения статуса задачи происходят только за счёт фоновых воркеров.
