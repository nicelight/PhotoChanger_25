...
Необязательные поля можно опускать — сервер обновляет только переданные значения. Успешный ответ `200 OK` возвращает структуру, идентичную `GET /api/settings` (без раскрытия секретов). Раздел `media_cache` доступен только для чтения: TTL обработанных результатов фиксирован (72 часа), а временных ссылок — 60 секунд. Попытки изменить эти параметры приводят к ошибке `422 Unprocessable Entity`.
