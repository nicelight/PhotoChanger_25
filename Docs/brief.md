...
* Значения TTL (`processed_media_ttl_hours`, `public_link_default_ttl_sec`, `max_manual_ttl_sec`) и другие не чувствительные параметры хранятся в `app_settings` с ключами `media.processed_ttl_hours`, `media.public_link_default_ttl_sec`, `media.max_manual_ttl_sec` и аналогичными. Для таких записей `is_secret = false`, а `value_json` содержит примитив (`integer`, `string`) или объект с описанием параметра. Эти настройки распространяются только на `media_object`; `template_media` хранится без TTL и очищается вручную. Миграции должны обеспечить начальные значения (72, 900 и 86400 соответственно) и поддерживать уникальность ключей. Если для удобства UI требуется отображать метаданные провайдеров без расшифровки (`project_id`, `region` и т. п.), используются отдельные записи `provider.<id>.<field>` с `is_secret = false`.
...
6. **Тест**: если загружено тест-фото — вспышка-анимация формы и тост «успех», иначе тост-ошибка.
7. **Сохранить**: визуальный «блик» формы; реального POST нет (submit отключён).

##### Модули и логика (верхний уровень) 

* Форма `#upload-form`
Хранит все поля. Submit заблокирован (`onsubmit="return false"`), кнопка «Сохранить» — `type="button"`. 
...
* Анимация формы
Один кейфрейм `@keyframes pulseForm` (opacity 1 → 0.08 → 1, 0.4s) и один класс `-pulse`; хелпер `pulse(el)` перезапускает анимацию. Привязано к «Тест» (только при наличии фото) и «Сохранить». 
* Доступность
