...
- **UI.** Статические страницы на Vanilla JS (Fetch) в `frontend/`, обращаются к REST-API модулей без лишних прослоек. UI не организует сложных параллельных операций.
