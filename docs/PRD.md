...
Система реализует ingest-интерфейс, хранение медиа, адаптацию вызовов провайдеров и публикацию результатов через временные публичные ссылки (TTL 168 ч).
...
- **Auth-сервис** — выдаёт JWT (HS256) для статических админов (`serg`, `igor`), токены scope `admin` действуют 168 ч и защищены throttling-политикой 10 попыток → блок 15 мин.
...
| **Пользователи публичных ссылок** | Загружают результаты по `GET /public/results/{job_id}` (TTL 168 ч). |
...
- **KPI итерации.**
  ...
  - Итоговые файлы доступны 168 часов, после чего автоматически удаляются.
...
- Долгосрочное архивирование результатов > 168 часов.
...
5. Публичные ссылки `/public/results/{job_id}` действуют 168 часов и возвращают `410 Gone` после истечения срока.
6. Cron-скрипт `scripts/cleanup_media.py` удаляет просроченные файлы и помечает записи в БД.
7. Публичная галерея доступна по статичному URL `/pubgallery`, активируется администратором на 15 минут через админ-UI; в это время действует rate limit 30 запросов в минуту и кэш ответа на 30–60 секунд.
...
### Администратор
1. Авторизуется через `POST /api/login` (статические аккаунты `serg`/`igor`), получает JWT со scope `admin` (TTL 168 ч); UI сохраняет токен в localStorage и добавляет `Authorization: Bearer ...` ко всем админ-запросам.
...
4. При успешном ответе драйвера файл переносится в `ResultStore`, запись `job_history` обновляется статусом `done`, `result_expires_at = now + 168h`, формируется ссылка `/public/results/{job_id}`.
...
- **Файловая система**:
  - `media/results/{slot_id}/{job_id}/payload.{ext}` (TTL = 168 часов).
...
#### Админ-API
- `POST /api/login` — получение JWT (scope `admin`, TTL 168 ч, throttling 10 попыток → блокировка на 15 мин).
...
- `GET /api/stats/slots` — активные слоты с KPI для графиков (`success_last_window`, `success_rate`, `timeout_rate`), только простые счётчики без тяжёлых агрегаций.

Все маршруты, кроме `POST /api/login`, требуют заголовка `Authorization: Bearer <token>` с действующим JWT scope `admin`.
...
* В `IngestService` результат принимается как base64 → сохраняется файл `MEDIA_ROOT/results/<job_id>.png` (или по MIME), обновляются `job_history`/`media_object` и `result_expires_at = now + 168h`.
...
- **Технология.** Статические страницы на чистом Vanilla JS + Fetch в `frontend/`, взаимодействуют с REST API: JSON для CRUD, `FormData` только там, где требуется загрузка файлов.
- **UI-правила:**
...
- Добавить отображение прогресса `POST /api/slots/{slot_id}/test-run` (стрим или polling).
- Серверная валидация размеров/типов и возврат статуса/ID уже реализуются REST API; документацию синхронизировать перед релизом.
...
Полный пример интерфейса теперь лежит в статическом наборе страниц `frontend/slots/slot-001.html`…`slot-015.html` (общие ресурсы в `frontend/slots/assets/`).
...
- **Успешные задачи** — `success_last_window`.
...
Для блоков графиков/виджетов страница дополнительно вызывает `GET /api/stats/slots`, который возвращает метрики только по активным слотам и содержит производные KPI:
...
- `success_rate` = `success_last_window / jobs_last_window` (0…1).
- `timeout_rate` = `timeouts_last_window / jobs_last_window`.
- Неактивные слоты и слоты без задач в окне в выдачу не попадают, чтобы не перегружать графики.
...
- JWT авторизация админов: `POST /api/login` выдаёт HS256-токен scope `admin` (TTL 168 ч). Токен хранится в браузере (localStorage) и передаётся как `Authorization: Bearer <token>` для всех `/api/slots*`, `/api/settings*`, `/api/stats*` и `/api/slots/{slot_id}/test-run`. Хэши статических аккаунтов лежат в `secrets/runtime_credentials.json`.
- Пароли ingest хранятся в plaintext (таблица `settings`/`slot`), проверяются строковым сравнением; в логи не выводятся.
