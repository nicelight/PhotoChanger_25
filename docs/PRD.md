...
- **Обязательные поля:**
  - `password` — строка.
  - `fileToUpload` — бинарный файл изображения. Поддерживаемые форматы: JPEG, PNG, WEBP. Лимит размера конфигурируемый (значение по умолчанию ≤ 15 МБ, чтобы соответствовать строгим ограничениям провайдера с минимальным порогом). 
