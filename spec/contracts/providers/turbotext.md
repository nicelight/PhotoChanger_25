...
- В нашей интеграции входные изображения передаются напрямую в `create_queue` как файлы `multipart/form-data`; публичные URL не используются.
...
- Возвращаемый `result_url`/`uploaded_image` скачивается адаптером сразу после ответа и сохраняется локально; последующий публичный доступ PhotoChanger не предоставляет.
...
- `Slot.settings_json` хранит параметры операций и идентификаторы `template_media`. Адаптер читает локальные пути/ID и формирует файловые вложения для `create_queue` без публикации URL.
- Полученный `result_url` скачивается, сохраняется в `media/results/{job_id}` и фиксируется в `job_history`.
- Очистка временных файлов (`media_object scope=provider`) выполняется после завершения работы либо cron-скриптом; повторное использование запрещено.
