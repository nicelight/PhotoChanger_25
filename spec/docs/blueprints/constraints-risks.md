...
- Администраторы готовы вручную продлевать временные ссылки при необходимости (например, для Turbotext), автоматизация не требуется на MVP.
...
| Утечка публичных ссылок | Временные URL доступными после завершения задачи. | TTL ≤ 50 сек, автоматическая очистка при финальных статусах, аудит продлений `/api/media/extend`. |
