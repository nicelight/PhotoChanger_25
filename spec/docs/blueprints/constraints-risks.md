...
- Воркеры исполняются внутри того же процесса FastAPI: при старте создаётся четыре асинхронных задачи `asyncio`, разделяющих event loop uvicorn; graceful shutdown должен останавливать принятие новых Job, отменять активные запросы и закрывать HTTP-клиенты.
...
| Падение воркера/очереди | Задачи застревают в статусе `pending`. | Heartbeat и автоперезапуск воркеров, idempotent подбор задач (PG `FOR UPDATE SKIP LOCKED`), контроль shutdown одного процесса FastAPI с четырьмя воркерами и проверка, что при рестарте фоновые задачи поднимаются повторно. |
