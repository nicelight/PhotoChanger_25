...
- Итоговый файл (`result_file_path`) доступен по публичной ссылке `GET /public/results/{job_id}` до `result_expires_at = finalized_at + T_result_retention`. Очиститель удаляет файл и обнуляет путь после наступления TTL, а endpoint возвращает `410 Gone`.
