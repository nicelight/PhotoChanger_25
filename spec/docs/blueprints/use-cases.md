...
- **Основной поток:**
  ...
  5. Admin API использует `SettingsService.get_settings(force_refresh=False)` как единственный публичный способ чтения конфигурации;
     метод `read_settings()` исключён из контракта и остаётся только во внутренних заметках миграции.
- **Ошибки:**
