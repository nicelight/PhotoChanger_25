...
- Публичное медиа-хранилище должно гарантировать доступность временных ссылок полный вычисленный TTL (`T_public_link_ttl = clamp(T_sync_response, 45, 60)`); дедлайн `Job.expires_at` фиксируется как `max(T_sync_response, T_public_link_ttl)`, чтобы очистка и фоновые воркеры не завершали задачу раньше этого окна.【F:Docs/brief.md†L20-L53】
