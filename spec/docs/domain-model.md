...
| JobHistory | `job_id`, `slot_id`, `source`, `status`, `failure_reason`, `started_at`, `completed_at`, `result_path`, `result_inline_base64`, `result_expires_at`, `metrics` | Фиксирует каждое обращение Ingest API. `source` отделяет боевые ingest (`ingest`) от запусков админов (`ui_test`, далее — cron/другие интеграции). Статусы: `pending`, `done`, `timeout`, `failed`. `result_expires_at = started_at + T_result_retention`. |
