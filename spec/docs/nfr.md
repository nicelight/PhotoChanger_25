...
- Размер загружаемого файла ограничен 15 МБ (жёсткий лимит FastAPI 2 ГБ); проверка MIME: JPEG, PNG, WEBP.
- `/api/slots`, `/api/settings`, `/api/stats/overview` отвечают <1 с при номинальной нагрузке (до 5 одновременных админов).
...
- Итоговые результаты доступны 168 ч; очистка просрочки завершается ≤15 мин после TTL.
- Бэкап PostgreSQL ежедневно; снапшоты `media/results` перед релизом.

## 3. Безопасность
- админ JWT подписывается `JWT_SIGNING_KEY`.
- Каждый ingest-запрос содержит обязательный `hash` (hex checksum); сервис пересчитывает хэш файла и сравнивает с переданным значением до начала обработки.
- Ingest пароль хранится в settings в plaintext, доступен только авторизованным админам через `/api/settings`; запрещено логировать значение, требуется TLS.
...
- `/metrics` публикует p95 ingest, долю 504, размер `media/results`, счётчик cron-очистки.
