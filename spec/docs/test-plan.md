...
> Операционный чеклист для запуска тестов: см. `docs/runbooks/test_playbook.md`.
...
- Фокус на проверке SLA (`T_sync_response`, TTL медиа 168 ч), корректности ошибок 504/502, работоспособности управления слотами и шаблонными медиа.

## 2. Среды тестирования
- **Local**: разработчики запускают `uvicorn src.app.main:app --reload` + локальную PostgreSQL (docker-compose), временный каталог результатов в `./media/results`.
...
- Дополнительные проверки: ротация ingest-пароля, валидация MIME/размеров (позитив: image/jpeg|png|webp; негатив: `Content-Type=image/gif` → 415, перегруз > лимита слота → 413 c `failure_reason=payload_too_large`), проверка удаления каталога `media/results/{slot}/{job}` при таймауте, очистка просроченных публичных ссылок (включая 404 → `result_not_found`, 410 → `result_expired`), метрики `/metrics`.
