...
- Дополнительные проверки: ротация ingest-пароля, валидация MIME/размеров (позитив: image/jpeg|png|webp; негатив: `Content-Type=image/gif` → 415, перегруз > лимита слота → 413 c `failure_reason=payload_too_large`), проверка удаления каталога `media/results/{slot}/{job}` при таймауте, очистка просроченных публичных ссылок, метрики `/metrics`.
