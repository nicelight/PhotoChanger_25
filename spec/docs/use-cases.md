...
- **Основной поток:**
  1. `POST /api/login` — получение JWT со scope `admin`; токен действует 168 ч и хранится в браузере (localStorage).
  2. `GET /api/settings` — просмотр текущих параметров (`T_sync_response`, TTL, дата/автор последних изменений).
  3. `PUT /api/settings` с новым `ingest.dslr_password` — сервис сохраняет пароль, обновляет метаданные и фиксирует событие в аудите.
- **Альтернативы:** `401 Unauthorized` — неверные креды или отсутствует токен; `429 Too Many Requests` — превышен лимит попыток; `403 Forbidden` — scope токена не `admin` (учётка отключена).
...
- **Предусловия:** JWT со scope `admin`.
- **Основной поток:**
  1. `GET /api/slots` — список слотов (id, имя, провайдер, активность, версия, `updated_at`).
  ...
  4. `PUT /api/slots/{slot_id}` — Slots API валидирует схему и лимиты провайдера (size limit, `sync_response_seconds`), сохраняет изменения и обновляет версию.
...
- **Основной поток:**
  ...
  6. Результат сохраняется в `media/results/{slot_id}/{job_id}/payload.{ext}` вместе с превью `preview.webp`; `job_history` обновляется (`status='done'`, `result_expires_at = now + 168h`), метрики фиксируются.
...
- **Итог:** оператор получает готовое изображение; результат доступен 168 ч через публичную ссылку; статистика обновлена.
...
- **Цель:** гарантировать автоматическое удаление результатов и публичных ссылок по истечении 168 ч.
...
- **Предусловия:** JWT со scope `admin`; учтены ограничения провайдера по форматам.
- **Основной поток:**
...
- **Основной поток:**
  ...
  2. `GET /api/stats/overview` — получение сводных метрик (jobs/timeout/provider_errors за окно + состояние слотов).
...
## UC7. Временная публичная галерея
- **Цель:** предоставить гостям доступ к галерее всех слотов на 15 минут без JWT.
- **Акторы:** Администратор, Гость, Public Gallery API.
- **Предусловия:** администратор авторизован в админ-UI.
- **Основной поток:**
  1. В админ-галерее нажимает «Расшарить на 15 минут» → `POST /api/gallery/share` включает флаг `share_until = now+15m` (in-memory).
  2. Гостю выдаётся статичная ссылка `/pubgallery`.
  3. Страница `/pubgallery` запрашивает `/pub/gallery`, получает агрегат по 15 слотам (до 10 результатов на слот, `latest_result` в топе).
  4. Rate limit 30 req/мин/клиент и кэш ответа на 30–60 сек защищают от спама.
  5. По истечении 15 минут `/pub/gallery` возвращает `403`, UI показывает сообщение об истечении.
- **Альтернативы:** `429` — превышен лимит; `503` — временная недоступность БД/файлов.
- **Итог:** гости видят свежие результаты в ограниченное окно; администратор может снова включить доступ.
