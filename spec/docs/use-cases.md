...
## Общие требования
- Ingest-поток принимает `multipart/form-data`, обязательные поля формы — `password` (строка) и `fileToUpload` (JPEG/PNG/WebP, размер ≤ лимита слота). Дополнительные поля DSLR Remote Pro передаёт произвольно, система их не ограничивает.
...
- Cron `scripts/cleanup_media.py` запускается каждые 15 минут и удаляет просроченные `media/temp` и `media/results` файлы, а также обновляет `media_object.cleaned_at`.
...
- **Основной поток:**
  1. `POST /api/ingest/{slot_id}` — multipart с обязательными `password` и `fileToUpload`; дополнительные поля принимаются без ограничений.
  2. Ingest API проверяет существование слота, сверяет пароль с хэшем, валидирует размер/MIME, захватывает семафоры (пер-слот и глобальный).
  3. Исходный файл сохраняется в `media/temp/{slot_id}/{job_id}`; создаётся `job_history(status='pending')` и временный `media_object`.
  ...
  6. Результат переносится в `media/results/{job_id}.{ext}`, временные файлы удаляются, `job_history` обновляется (`status='done'`, `result_expires_at = now + 72h`), метрики фиксируются.
...
- **Основной поток / отклонение:**
  ...
  3. `IngestService` обновляет `job_history(status='timeout', failure_reason='provider_timeout')`, удаляет временные файлы/ссылки и освобождает семафоры.
...
- **Альтернативы:** `404 Not Found` — неверный `job_id` или файл отсутствует; `503` — временная недоступность хранилища.
...
- **Основной поток:**
  ...
  4. Кнопка «Очистить медиакеш» вызывает `POST /api/slots/{slot_id}/cleanup`, что инициирует отложенную очистку временных файлов (актуальные результаты не удаляются).
