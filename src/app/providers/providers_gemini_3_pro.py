"""Gemini 3 Pro Image Preview provider driver implementation."""
...
class Gemini3ProDriver(ProviderDriver):
    """Call Gemini 3 Pro Image Preview via generateContent (inline_data)."""
    ...
    async def process(self, job: JobContext) -> ProviderResult:
        ...
        if not prompt:
            raise ProviderExecutionError(
                "Gemini 3 Pro prompt is required in slot settings"
            )

        model = settings.get("model", "gemini-3-pro-image-preview")
        ...
        image_config_payload = _build_image_config(image_config)
        ...
        while True:
            ...
            self.log.info("gemini3.response.received %s", summary)
            ...
            if not has_inline:
                ...
                self.log.warning(
                    "gemini3.response.no_inline_data %s",
                    body_preview,
                    extra={
                        "slot_id": job.slot_id,
                        "job_id": job.job_id,
                ...
                if "NO_IMAGE" in finish_reasons:
                    no_image_attempts += 1
                    self.log.warning(
                        "gemini3.response.no_image attempt=%s/%s finish_reasons=%s",
                        no_image_attempts,
                    ...
                    if no_image_attempts >= NO_IMAGE_MAX_ATTEMPTS:
                        raise ProviderTimeoutError(
                            "Gemini 3 Pro returned NO_IMAGE after retries"
                        )
                    ...
                    if (
                        ...
                        raise ProviderTimeoutError(
                            "Gemini 3 Pro NO_IMAGE retry would exceed sync deadline"
                        )
                ...
                raise ProviderExecutionError(
                    f"Gemini 3 Pro response has no image (finish_reason={reason})"
                )

            result = self._parse_response(data, fallback_mime=(output or ingest_mime))
            self.log.info(
                "gemini3.request.success",
                extra={"slot_id": job.slot_id, "job_id": job.job_id},
            )
            return result

        raise ProviderExecutionError("Gemini 3 Pro request failed after retries")
        ...
        *,
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if not self._should_retry(response) or attempt >= max_attempts:
                ...
                self.log.error(
                    "gemini3.response.error status=%s detail=%s body_preview=%s",
                    ...
                    extra={
                        ...
                        "provider_error_message": error_detail,
                    },
                )
                raise ProviderExecutionError(
                    f"Gemini 3 Pro request failed (status={response.status_code}): {error_detail}"
                )
            ...
            for part in content.get("parts", []):
                ...
                if inline and inline.get("data"):
                    ...
                    try:
                        ...
                        raise ProviderExecutionError(
                            "Gemini 3 Pro response payload is invalid"
                        ) from exc
                    return ProviderResult(payload=payload, content_type=mime)
        raise ProviderExecutionError("Gemini 3 Pro response does not contain inline data")


def _build_image_config(settings: dict[str, Any]) -> dict[str, str]:
    config: dict[str, str] = {}
