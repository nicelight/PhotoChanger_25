...
from src.app.providers.providers_gemini_3_pro import Gemini3ProDriver
...
def job_context(tmp_path: Path) -> JobContext:
    ...
    job.slot_settings = {
        ...
        "prompt": "Create a clean portrait",
...
async def test_process_success(
    ...
    driver = Gemini3ProDriver(media_repo=media_repo)
    ...
    assert (
        client.requests[0]["json"]["generationConfig"]["imageConfig"]["aspectRatio"]
        == "16:9"
    )
    assert (
        client.requests[0]["json"]["generationConfig"]["imageConfig"]["imageSize"]
        == "2K"
    )
